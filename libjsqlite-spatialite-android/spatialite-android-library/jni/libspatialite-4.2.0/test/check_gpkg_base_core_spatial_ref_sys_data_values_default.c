...
int main (int argc UNUSED, char *argv[] UNUSED)
{
    ...
    ret = sqlite3_get_table (db_handle, "SELECT srs_id, organization, organization_coordsys_id, definition FROM gpkg_spatial_ref_sys WHERE srs_id = -1", &results, &rows, &columns, &err_msg);
    if (ret != SQLITE_OK) {
    ...
    {
	fprintf (stderr, "Unexpected srs_id result (got %s, expected -1)", results[1 * columns + 0]);
	sqlite3_free_table (results);
	return -11;
    }
    if (strcmp(results[1 * columns + 1], "NONE") != 0)
    {
	fprintf (stderr, "Unexpected organization result (got %s, expected NONE)", results[1 * columns + 1]);
	sqlite3_free_table (results);
	return -12;
    }
    if (strcmp(results[1 * columns + 2], "-1") != 0)
    {
	fprintf (stderr, "Unexpected organization_coordsys_id result (got %s, expected -1)", results[1 * columns + 2]);
	sqlite3_free_table (results);
	return -13;
    }
    if (strcmp(results[1 * columns + 3], "Undefined") != 0)
    {
	fprintf (stderr, "Unexpected definition result (got %s, expected Undefined)", results[1 * columns + 3]);
	sqlite3_free_table (results);
	return -14;
    }
    sqlite3_free_table (results);
    
    ret = sqlite3_get_table (db_handle, "SELECT srs_id, organization, organization_coordsys_id, definition FROM gpkg_spatial_ref_sys WHERE srs_id = 0", &results, &rows, &columns, &err_msg);
    if (ret != SQLITE_OK) {
    ...
    {
	fprintf (stderr, "Unexpected srs_id result (got %s, expected 0)", results[1 * columns + 0]);
	sqlite3_free_table (results);
	return -22;
    }
    if (strcmp(results[1 * columns + 1], "NONE") != 0)
    ...
    {
	fprintf (stderr, "Unexpected organization_coordsys_id result (got %s, expected 0)", results[1 * columns + 2]);
	sqlite3_free_table (results);
	return -24;
    }
    if (strcmp(results[1 * columns + 3], "Undefined") != 0)
    ...
    ret = sqlite3_get_table (db_handle, "SELECT srs_id, organization, organization_coordsys_id, definition FROM gpkg_spatial_ref_sys WHERE srs_id = 4326", &results, &rows, &columns, &err_msg);
    if (ret != SQLITE_OK) {
    ...
    if ((rows != 1) || (columns != 4))
    {
	fprintf (stderr, "Unexpected row / column count (srs_id4326): %i x %i\n", rows, columns);
	sqlite3_free_table (results);
	return -31;
    }
    if (strcmp(results[1 * columns + 0], "4326") != 0)
    {
	fprintf (stderr, "Unexpected srs_id result (got %s, expected 4326)", results[1 * columns + 0]);
	sqlite3_free_table (results);
	return -32;
    }
    if (strcasecmp(results[1 * columns + 1], "epsg") != 0)
    {
	fprintf (stderr, "Unexpected organization result (got %s, expected epsg)", results[1 * columns + 1]);
	sqlite3_free_table (results);
	return -33;
    }
    if (strcmp(results[1 * columns + 2], "4326") != 0)
    {
	fprintf (stderr, "Unexpected organization_coordsys_id result (got %s, expected 4326)", results[1 * columns + 2]);
	sqlite3_free_table (results);
	return -34;
    }
    if (strcmp(results[1 * columns + 3], "GEOGCS[\"WGS 84\",DATUM[\"WGS_1984\",SPHEROID[\"WGS 84\",6378137,298.257223563,AUTHORITY[\"EPSG\",\"7030\"]],AUTHORITY[\"EPSG\",\"6326\"]],PRIMEM[\"Greenwich\",0,AUTHORITY[\"EPSG\",\"8901\"]],UNIT[\"degree\",0.0174532925199433,AUTHORITY[\"EPSG\",\"9122\"]],AUTHORITY[\"EPSG\",\"4326\"]]") != 0)
    {
	fprintf (stderr, "Unexpected definition result (got %s, expected GEOGCS[...])", results[1 * columns + 3]);
	sqlite3_free_table (results);
	return -35;
    }
    sqlite3_free_table (results);
