...
int main (int argc UNUSED, char *argv[] UNUSED)
{
    ...
    sql_statement = "SELECT sql FROM sqlite_master WHERE type = 'table' AND tbl_name = 'gpkg_extensions'";
	...
	if (strncasecmp(token, "CONSTRAINT", strlen("CONSTRAINT")) == 0)
	{
	    ...
	    char *fulltoken = malloc(strlen(token) + strlen(",") + strlen(nexttoken1) + strlen(",") + strlen(nexttoken2) + 1);
	    strcpy(fulltoken, token);
	    strcat(fulltoken, ",");
	    strcat(fulltoken, nexttoken1);
	    strcat(fulltoken, ",");
	    strcat(fulltoken, nexttoken2);
	    if (strcasestr(fulltoken, "UNIQUE (table_name, column_name, extension_name)"))
	    {
