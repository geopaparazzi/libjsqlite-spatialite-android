...
int main (int argc UNUSED, char *argv[] UNUSED)
{
    ...
    // Test Type Basic
    sql_statement = "SELECT sql FROM sqlite_master WHERE type = 'table' AND tbl_name = 'gpkg_contents'";
	...
	for (i = 0; tableValues[i].column_name != NULL; ++i)
	{
	    ...
	    {
		if (strcasestr(token, "FOREIGN KEY (srs_id) REFERENCES gpkg_spatial_ref_sys(srs_id)"))
		{
		...
		{
		    fprintf(stderr, "missing COLUMN TYPE %s for %s: %s\n", tableValues[i].column_type, tableValues[i].column_name, token);
		    return -20;
		}
		if (tableValues[i].mayBeNull && strcasestr(token, "NOT NULL"))
		{
		    fprintf(stderr, "Unexpected NOT NULL constraint for %s: %s\n", tableValues[i].column_name, token);
		    return -21;
		}
		if ((!tableValues[i].mayBeNull) && (!strcasestr(token, "NOT NULL")))
		{
		    fprintf(stderr, "Missing NOT NULL constraint for %s: %s\n", tableValues[i].column_name, token);
		    return -22;
		}
		if ((tableValues[i].keySignature != NULL) && (!strcasestr(token, tableValues[i].keySignature)))
		{
		    fprintf(stderr, "Missing %s constraint for %s: %s\n", tableValues[i].keySignature, tableValues[i].column_name, token);
		    return -23;
		}
		if ((tableValues[i].keySignature == NULL) && (strcasestr(token, "KEY")))
		{
		    fprintf(stderr, "Unexpected key constraint for %s: %s\n", tableValues[i].column_name, token);
		    return -24;
		}
		if ((tableValues[i].defaultValue == NULL) && (strcasestr(token, "DEFAULT")))
		{
		    fprintf(stderr, "Unexpected default value for %s: %s\n", tableValues[i].column_name, token);
		    return -25;
		}
		if ((tableValues[i].defaultValue != NULL) && (!strcasestr(token, tableValues[i].defaultValue)))
		{
		    fprintf(stderr, "Missing default value for %s: %s\n", tableValues[i].column_name, token);
		    return -26;
		}
	...
	if (strcasestr(token, "last_change "))
	{
	    ...
	    char *fulltoken = malloc(strlen(token) + strlen(",") + strlen(nexttoken) + 1);
	    strcpy(fulltoken, token);
	    strcat(fulltoken, ",");
	    strcat(fulltoken, nexttoken);
	    if (!strcasestr(fulltoken, "TEXT"))
	    {
		fprintf(stderr, "missing COLUMN TYPE TEXT for last_change: %s\n", fulltoken);
		free(fulltoken);
		return -40;
	    }
	    if (!strcasestr(fulltoken, "NOT NULL"))
	    {
		fprintf(stderr, "Missing NOT NULL constraint for last_change: %s\n", fulltoken);
		free(fulltoken);
		return -41;
	    }
	    if (strcasestr(fulltoken, "KEY"))
	    {
		fprintf(stderr, "Unexpected KEY constraint for last_change: %s\n", token);
		free(fulltoken);
		return -42;
	    }
	    if (!strcasestr(fulltoken, "DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ',CURRENT_TIMESTAMP))"))
	    {
		fprintf(stderr, "Missing default value for last_change: %s\n", fulltoken);
		free(fulltoken);
		return -43;
	    }
	    free(fulltoken);
    ...
    {
        fprintf(stderr, "did not find expected last_change column, or required properties were not found\n");
	return -51;
    }
