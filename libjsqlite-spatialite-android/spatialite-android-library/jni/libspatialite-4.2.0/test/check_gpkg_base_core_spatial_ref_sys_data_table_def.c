...
int main (int argc UNUSED, char *argv[] UNUSED)
{
    ...
    sql_statement = "SELECT sql FROM sqlite_master WHERE type = 'table' AND tbl_name = 'gpkg_spatial_ref_sys'";
    ...
    sql = strdup(strstr(sqlite3_column_text(stmt, 0),  "(") + strlen("("));
    if (!sql)
    {
	...
	if (strstr(token, "srs_name "))
	{
	    ...
	    {
	       fprintf(stderr, "missing COLUMN TYPE TEXT for srs_name: %s\n", token);
	       return -34;
	    }
	    if (!strstr(token, "NOT NULL"))
	    {
	        fprintf(stderr, "Missing NOT NULL constraint for srs_name: %s\n", token);
		return -35;
	    }
	    if (strstr(token, "KEY"))
	    {
	        fprintf(stderr, "Unexpected KEY constraint for srs_name: %s\n", token);
		return -36;
	    }
	    valid_srs_name = true;
	}
	if (strstr(token, "srs_id "))
	{
	    ...
	    {
	       fprintf(stderr, "missing COLUMN TYPE INTEGER for srs_id: %s\n", token);
	       return -37;
	    }
	    if (!strstr(token, "NOT NULL"))
	    {
	        fprintf(stderr, "Missing NOT NULL constraint for srs_id: %s\n", token);
		return -38;
	    }
	    if (!strstr(token, "PRIMARY KEY"))
	    {
	        fprintf(stderr, "Missing PRIMARY KEY constraint for srs_id: %s\n", token);
		return -39;
	    }
	    valid_srs_id = true;
	}
	if (strstr(token, "organization "))
	{
	    if (!strstr(token, "TEXT"))
	    {
	       fprintf(stderr, "missing COLUMN TYPE TEXT for organization: %s\n", token);
	       return -40;
	    }
	    if (!strstr(token, "NOT NULL"))
	    {
	        fprintf(stderr, "Missing NOT NULL constraint for organization: %s\n", token);
		return -41;
	    }
	    if (strstr(token, "KEY"))
	    {
	        fprintf(stderr, "Unexpected KEY constraint for organization: %s\n", token);
		return -42;
	    }
	...
	if (strstr(token, "organization_coordsys_id "))
	{
	    if (!strstr(token, "INTEGER"))
	    {
	       fprintf(stderr, "missing COLUMN TYPE INTEGER for organization_coordsys_id: %s\n", token);
	       return -43;
	    }
	    if (!strstr(token, "NOT NULL"))
	    {
	        fprintf(stderr, "Missing NOT NULL constraint for organization_coordsys_id: %s\n", token);
		return -44;
	    }
	    if (strstr(token, "KEY"))
	    {
	        fprintf(stderr, "Unexpected PRIMARY KEY constraint for organization_coordsys_id: %s\n", token);
		return -45;
	    }
	...
	if (strstr(token, "definition "))
	{
	    if (!strstr(token, "TEXT"))
	    {
	       fprintf(stderr, "missing COLUMN TYPE TEXT for definition: %s\n", token);
	       return -46;
	    }
	    if (!strstr(token, "NOT NULL"))
	    {
	        fprintf(stderr, "Missing NOT NULL constraint for definition: %s\n", token);
		return -47;
	    }
	    if (strstr(token, "KEY"))
	    {
	        fprintf(stderr, "Unexpected KEY constraint for definition: %s\n", token);
		return -48;
	    }
	...
	if (strstr(token, "description "))
	{
	    if (!strstr(token, "TEXT"))
	    {
	       fprintf(stderr, "missing COLUMN TYPE TEXT for description: %s\n", token);
	       return -49;
	    }
	    if (strstr(token, "NOT NULL"))
	    {
	        fprintf(stderr, "Unexpected NOT NULL constraint for description: %s\n", token);
		return -50;
	    }
	    if (strstr(token, "KEY"))
	    {
	        fprintf(stderr, "Unexpected KEY constraint for description: %s\n", token);
		return -51;
	    }
    ...
    if (! valid_srs_name)
    {
        fprintf(stderr, "did not find expected srs_name column, or required properties were not found\n");
	return -99;
    }
    if (! valid_srs_id)
    {
        fprintf(stderr, "did not find expected srs_id column, or required properties were not found\n");
	return -100;
    }
    if (! valid_organization)
    {
        fprintf(stderr, "did not find expected organization column, or required properties were not found\n");
	return -101;
    }
    if (! valid_organization_coordsys_id)
    {
        fprintf(stderr, "did not find expected organization_coordsys_id column, or required properties were not found\n");
	return -102;
    }
    if (! valid_definition)
    {
        fprintf(stderr, "did not find expected definition column, or required properties were not found\n");
	return -103;
    }
    if (! valid_description)
    {
        fprintf(stderr, "did not find expected description column, or required properties were not found\n");
	return -104;
    }
