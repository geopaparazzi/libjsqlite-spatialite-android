...
/* performing EXPORT */
    ...
    if (cvg == NULL)
	return 0;
    // TODO: support for cutline logic
    // - based on gdalwarp, 3 parameters could be used:
    // -- '-cutline datasource', '-csql statement' and '-crop_to_cutline'
    // ----
    // - this 'datasource' should be queried with the 'statement'
    // -- the result stored in 'blob_cutline' and 'blob_cutline_sz' and passed to 'rl2_resolve_image_sanity'
    // ----
    const unsigned char *blob_cutline=NULL;
    int blob_cutline_sz=0;
    int crop_to_cutline=0; 
    unsigned char **geom_cutline_blob=malloc(sizeof(char *));
    *geom_cutline_blob=NULL; 
    int *geom_cutline_blob_sz=malloc(sizeof(int));
    *geom_cutline_blob_sz=0; 
 int i_rc=0;
   int srid;
   rl2_get_coverage_srid (cvg, &srid);
    if (base_resolution)
      {
	  ...
	  char *dumb2;
	  int err_bbox = 0;
      /* Attempt to resolve missing parameters, were possible */
    i_rc=rl2_resolve_image_sanity(handle,coverage, NULL, 0, &minx, &miny, &maxx, &maxy,&x_res,&y_res,&width,&height,&cx,&cy,
             blob_cutline,blob_cutline_sz,crop_to_cutline,geom_cutline_blob,geom_cutline_blob_sz,srid);
    if (i_rc != RL2_OK)
    {
     err_bbox = 1;
     switch (i_rc)
     {
      case 100: // tie-point: Center Point
       fprintf (stderr, "*** ERROR *** undeclared Center-X and Center-Y [ tie-point: Center Point]\n");
      break;
      case 101:
       fprintf (stderr, "*** ERROR *** undeclared Center-X [ tie-point: Center Point]\n");
      break;
      case 102:
       fprintf (stderr, "*** ERROR *** undeclared Center-Y [ tie-point: Center Point]\n");
      break;
      case 105: // tie-point: LowerLeft Corner
       fprintf (stderr, "*** ERROR *** undeclared Min-X and Min-Y [tie-point: LowerLeft Corner]\n");
      break;
      case 106:
       fprintf (stderr, "*** ERROR *** undeclared Min-X [tie-point: LowerLeft Corner]\n");
      break;
      case 107:
       fprintf (stderr, "*** ERROR *** undeclared Min-Y [tie-point: LowerLeft Corner]\n");
      break;
      case 110: // tie-point: LowerRight Corner
       fprintf (stderr, "*** ERROR *** undeclared Max-X and Min-Y [tie-point: LowerRight Corner]\n");
      break;
      case 111:
       fprintf (stderr, "*** ERROR *** undeclared Max-X [tie-point: LowerRight Corner]\n");
      break;
      case 112:
       fprintf (stderr, "*** ERROR *** undeclared Min-Y [tie-point: LowerRight Corner]\n");
      break;
      case 115: // tie-point: UpperLeft Corner
       fprintf (stderr, "*** ERROR *** undeclared Min-X and Max-Y [tie-point: UpperLeft Corner]\n");
      break;
      case 116:
       fprintf (stderr, "*** ERROR *** undeclared Min-X [tie-point: UpperLeft Corner]\n");
      break;
      case 117:
       fprintf (stderr, "*** ERROR *** undeclared Max-Y [tie-point: UpperLeft Corner]\n");
      break;
      case 120: // tie-point: UpperRight Corner
       fprintf (stderr, "*** ERROR *** undeclared Max-X and Max-Y [tie-point: UpperRight Corner]\n");
      break;
      case 121:
       fprintf (stderr, "*** ERROR *** undeclared Max-X [tie-point: UpperRight Corner]\n");
      break;
      case 122:
       fprintf (stderr, "*** ERROR *** undeclared Max-Y [tie-point: UpperRight Corner]\n");
      break;
      case 125: // Pixel-size / resolution [not set]
       fprintf (stderr, "*** ERROR *** invalid X/Y pixel size [resolution not set]\n");
      break;
      case 126:
       fprintf (stderr, "*** ERROR *** invalid X pixel size [resolution not set]\n");
      break;
      case 127:
       fprintf (stderr, "*** ERROR *** invalid Y pixel size [resolution not set]\n");
      break;
      case 130: // Pixel-size / resolution [not set]
       fprintf (stderr, "*** ERROR *** invalid X/Y pixel size [resolution <= 0]\n");
      break;
      case 131:
       fprintf (stderr, "*** ERROR *** invalid X pixel size [resolution <= 0]\n");
      break;
      case 132:
       fprintf (stderr, "*** ERROR *** invalid Y pixel size [resolution <= 0]\n");
      break;
      default:
       fprintf (stderr, "*** ERROR ***invalid output image tie-point [cause unknowncause unknown: rc[%d], exec_export]\n",i_rc);      
      break;
     }
    }
	  if (err_bbox)
    ...
    if (is_ascii_grid (dst_path))
	  ...
	  if (rl2_export_ascii_grid_from_dbms
	      (handle, max_threads, dst_path, cvg, x_res, minx, miny, maxx,
	       maxy, width, height, 0, 4,*geom_cutline_blob,geom_cutline_blob_sz) != RL2_OK)
	      goto error;
      }
	 ...
	 int with_worldfile = 0;
	  ...
	  if (rl2_export_jpeg_from_dbms
	      (handle, max_threads, dst_path, cvg, x_res, y_res, minx, miny,
	       maxx, maxy, width, height, 85, with_worldfile,*geom_cutline_blob,geom_cutline_blob_sz) != RL2_OK)
	      goto error;
      }
    else
      {
	  ...
	  if (rl2_export_geotiff_from_dbms
	      (handle, max_threads, dst_path, cvg, x_res, y_res, minx, miny,
	       maxx, maxy, width, height, compression, 256, 0,*geom_cutline_blob,geom_cutline_blob_sz) != RL2_OK)
	      goto error;
      }
    ...
    if (is_ascii_grid (dst_path))
	  ...
	  if (rl2_export_section_ascii_grid_from_dbms
	      (handle, max_threads, dst_path, cvg, section_id, x_res, minx,
	       miny, maxx, maxy, width, height, 0, 4,NULL,NULL) != RL2_OK)
	      goto error;
      }
    else if (is_jpeg_image (dst_path))
      {
	  ...
	  if (rl2_export_section_jpeg_from_dbms
	      (handle, max_threads, dst_path, cvg, section_id, x_res, y_res,
	       ...
	       with_worldfile,NULL,NULL) != RL2_OK)
	      goto error;
      }
    else
      {
...
/* checking/printing EXPORT args */
    ...
    printf ("===========================================================\n");
    i_rc=rl2_resolve_image_sanity(NULL,coverage, NULL, 0, minx, miny, maxx, maxy,&x_res,&x_res,width,height,cx,cy,NULL,0,0,NULL,NULL,-1);

    /* Attempt to resolve missing parameters, where possible */
    if ((x_res == DBL_MAX) && ( y_res == DBL_MAX))
    { // resolution cannot be trieved due to missing Database connection
     base_resolution=1; // rerieve default resolution of the coverage [--base-resolution]
     no_res=1;
    }
    if (i_rc != RL2_OK)
    {
     ...
     switch (i_rc)
     {
      ...
      break;
      default:
       fprintf (stderr, "*** ERROR ***invalid output image tie-point [cause unknown: rc[%d], check_export_args]\n",i_rc);      
      break;
     }
    }
    if (db_path == NULL)
    ...
    if (!base_resolution)
      {
       if (*width == 0)
       {
	      fprintf (stderr, "*** ERROR *** NULL/ZERO image Width[%u]\n",*width);
	      err = 1;
	      err_bbox = 1;
      }
      if (*height == 0)
      {
	      fprintf (stderr, "*** ERROR *** NULL/ZERO image Height[%u]\n",*height);
	      err = 1;
	      err_bbox = 1;
      }
      if (!err)
  ...
  error:
    if (!base_resolution)
	  ...
	  if (err_bbox)
	    {
		fprintf (stderr,
			 "*** ERROR *** unable to determine the BBOX of output image [Min-X[%f],Min-Y[%f],Max-X[%f],Max-Y[%f]]\n",*minx,*miny,*maxx,*maxy);
		err = 1;
	    }
	  else
	    {
