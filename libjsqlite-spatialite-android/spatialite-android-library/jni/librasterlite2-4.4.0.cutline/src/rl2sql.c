...
/**
 ...
 - to read and reset the Zoom-Level and Center Point  alues
 
 \param coverage_name name of the coverage [the coverage MUST exist]
 \param 2nd Parameter as BLOB :  Center Point [must be inside the extent of the raster_coverage]
 \param 2nd Parameter as integer (with no further Parameters) :  Restore Default Values [0,1,2]
 \param 2nd Parameter as integer (2 further Parameters) :  zomm_min [0-30]
 \param 3nd Parameter as integer (1 further Parameter) :  zomm_default [0-30]
 \param 4nd Parameter as integer (last Parameter) :  zomm_max [0-30]
 
 \note Both the 'Center Point' and 'Zoom Levels' 
  
 */
static void
fnct_GetRasterCoverageDefaults (sqlite3_context * context, int argc,
			 sqlite3_value ** argv)
{
/* SQL function:
/ GetRasterCoverageDefaults(text coverage)
/ GetRasterCoverageDefaults(text coverage, int parm_type)
/ parm_type text
// 0 [default]
/  - Text representation of all parms, divided by 
...
/  parm_type 1 
/   center point as blob
/  parm_type 2 
/   center point as ewkt
/  parm_type 3 
/   zoom_min
/  parm_type 4 
/   zoom_default
...
*/
    ...
    int zoom_default=0;
    int zoom_max=0;
    unsigned char c_zoom_min=0;
    unsigned char c_zoom_default=0;
    unsigned char c_zoom_max=0;
    double parm_res_x=0.0;
    double parm_res_y=0.0;
    int parm_srid=0;
    double center_x=0.0;
    double center_y=0.0;
    int i_length=0;
    unsigned char *blob_center=NULL;
    int blob_center_sz=0;
    char *ewkt_center=NULL;
    char *converage_defaults_center=NULL;
    int i_debug=0;
    //i_debug=1;
    RL2_UNUSED ();		/* LCOV_EXCL_LINE */
    if ((sqlite3_value_type (argv[0]) != SQLITE_TEXT) && (sqlite3_value_type (argv[0]) != SQLITE_FLOAT) &&
         (sqlite3_value_type (argv[0]) != SQLITE_INTEGER))
    { // 1: SQLITE_INTEGER, 2: SQLITE_FLOAT, 3: SQLITE_TEXT,  4: SQLITE_BLOB, or 5:  SQLITE_NULL.
	    i_rc = -1; // 1st parameter not TEXT,FLOAT or INTEGER     
    }
    else
    {
     if (argc > 1) 
     {
      if (sqlite3_value_type (argv[1]) != SQLITE_INTEGER)
      {
	      i_rc=-2; // 2nd (optional Parameter) not an integer
       if (i_debug > 0)
        coverage_name=sqlite3_mprintf("sqlite3_value_type (argv[1]) [%d]",sqlite3_value_type (argv[1]));
      }    
     }
    }
    if (i_rc < RL2_OK)
    ...
    if (sqlite3_value_type (argv[0]) == SQLITE_TEXT)
    {
     command_type=0;
     parm_command=0;
     coverage_name= (char *) sqlite3_value_text(argv[0]);
     if (coverage_name == NULL)
      i_rc=-3; // 1st Parameter, coverage_name, is NULL
    }   
    if (sqlite3_value_type (argv[0]) == SQLITE_FLOAT)
    {
     command_type=1;
     parm_command=0;
     parm_res_x=sqlite3_value_double(argv[0]);
     if (parm_res_x < 0.0)
      parm_res_x = parm_res_x * -1;
     if (parm_res_x ==  0.0)
      i_rc=-4; // 1st Parameter, parm_res_x, is 0.0
    }
    if (sqlite3_value_type (argv[0]) == SQLITE_INTEGER)
    {
     command_type=2;
     parm_command=sqlite3_value_int(argv[0]);
     if ((parm_command < 0) || (parm_command > 5))
      i_rc=-5; // 1st Parameter, parm_command, not between 0-5
    }
    if (i_rc < 0)
     goto error; 
    if (argc > 1)
    {
	    parm_type = sqlite3_value_int (argv[1]);  
     if ((command_type == 0) || (command_type == 1))
     {
      if ((parm_type < 0) || (parm_type > 5))
      { // 2nd Parameter,parm_type, not between 0-5
       i_rc=-6;
       goto error;
      }
      if (command_type == 1)
      {
       if ((parm_type > 0) && (parm_type < 2))
        parm_type=0;
      }
     }
     if (command_type == 2)
     {
      if ((parm_type < 0) || (parm_type > 5))
      { // 2nd Parameter,parm_type of command_type=2, not between 0-?
       i_rc=-7;
       goto error;
      }
     }
     if (argc > 2) 
     {
      if (sqlite3_value_type (argv[2]) != SQLITE_INTEGER)
      {
	      i_rc=-8; // 3rd (optional Parameter) not an integer
      }
      else
      {
       if ((command_type == 1) || (command_type == 3))
       {
        parm_srid=sqlite3_value_int(argv[2]);
       }
      }
     }
    }
    ...
    {
     case 0:
     { // Tasks using coverage_name as first parameter
      cvg = rl2_create_coverage_from_dbms (sqlite, coverage_name);
      if (cvg == NULL)
      {
       i_rc=-10;
	      goto error;
      }
      i_rc=rl2_get_raster_coverage_stats(sqlite,coverage_name,&coverage_stats,&coverage_srid);
      if (i_rc == RL2_OK)
      { // SELECT RL2_GetRasterCoverageDefaults('1926.berlin_eiskeller.cutline',0) 
       coverage_raster_defaults=((rl2PrivRasterStatisticsPtr)coverage_stats)->raster_defaults;
       if (coverage_raster_defaults != NULL)
       {
        zoom_min=(int)coverage_raster_defaults->zoom_min;
        zoom_default=(int)coverage_raster_defaults->zoom_default;
        zoom_max=(int)coverage_raster_defaults->zoom_max;
        center_x=coverage_raster_defaults->center_x;
      ...
      { // retrieving Coverage RasterStatistics has failed
       i_rc+=-100;
      }
      if (i_rc < RL2_OK)
      	goto error;
      switch (parm_type)
      {
       ...
       case 2: 
       {
        sql=sqlite3_mprintf("SELECT MakePoint(?,?,%d), AsEWKT(MakePoint(?,?,%d))",coverage_srid,coverage_srid);
        i_sql_rc = sqlite3_prepare_v2 (sqlite, sql, strlen (sql), &stmt_ewkt, NULL);
        if (i_sql_rc == SQLITE_OK)
        {
         sqlite3_free(sql);
         sql=NULL;
         i_rc=-12;
         sqlite3_reset (stmt_ewkt);
         sqlite3_clear_bindings (stmt_ewkt);
         sqlite3_bind_double (stmt_ewkt, 1,center_x);
         sqlite3_bind_double (stmt_ewkt, 2,center_y);
         sqlite3_bind_double (stmt_ewkt, 3,center_x);
         sqlite3_bind_double (stmt_ewkt, 4,center_y);
         while (1)
         {
          i_sql_rc = sqlite3_step (stmt_ewkt);
	         if (i_sql_rc == SQLITE_DONE)
	          break;
          if (i_sql_rc == SQLITE_ROW)
	         {
           if (sqlite3_column_type (stmt_ewkt, 0) == SQLITE_BLOB)
		         {
		          unsigned char *blob = (unsigned char *)sqlite3_column_blob (stmt_ewkt, 0);
		          blob_center_sz = sqlite3_column_bytes (stmt_ewkt, 0);
            if (blob_center_sz > 0)
            {
             blob_center=malloc(blob_center_sz);
             memcpy(blob_center, blob, blob_center_sz);
            }
           }
           if (sqlite3_column_type (stmt_ewkt, 1) == SQLITE_TEXT)
		         {
            const char *str=(const char *)sqlite3_column_text(stmt_ewkt,1);
            if (str != NULL)
            {
             int len = strlen (str);
             ewkt_center=sqlite3_malloc(len+1);		       
             if (ewkt_center!=NULL)
             {
              strcpy (ewkt_center, str);
              // 'center_x,center_y,coverage_srid;zoom_min,zoom_default,zoom_max'
              sql=sqlite3_mprintf("%2.7f,%2.7f,%d;%d,%d,%d",center_x,center_y,coverage_srid,zoom_min,zoom_default,zoom_max);
              len = strlen (sql);
              converage_defaults_center=sqlite3_malloc(len+1);		
              strcpy (converage_defaults_center, sql);
              sqlite3_free(sql);
              sql=NULL;
              i_rc=RL2_OK;
             }
         ...
         sqlite3_finalize (stmt_ewkt);
         stmt_ewkt = NULL;
        }
        else
        { // sql prepared for SELECT MakePoint and  AsEWKT values has failed
         i_rc=-11;
         // printf ("-E->fnct_GetRasterCoverageDefaults[%s,%d] sql[%s] srid[%d]\n",coverage_name,i_rc,sql,coverage_srid); 
         sqlite3_free(sql);
         sql=NULL;
        }	
        if (sql != NULL)
        ...
        if (i_rc < RL2_OK)
        	goto error; 
        if ((ewkt_center==NULL) || (blob_center == NULL || (blob_center_sz <= 1)))
        { // At least one of the retrived  values from SELECT MakePoint and  AsEWKT values are NULL
         i_rc=-13;     
        }
       }
       break;     
     ...
     break;
     case 1:
     { // Tasks using parm_res_x as first parameter
      if (parm_srid < 0)
       parm_srid=187999;
      if (parm_srid == 0)
       parm_srid=3395;
      ...
      if ((parm_type == 0) || (parm_type == 2))
       i_parm_result=3;
      i_rc=rl2_get_zoom_levels_for_resolutions(sqlite,NULL,parm_srid,parm_res_x,parm_res_x,center_x,center_y,
                                                             &c_zoom_min,&c_zoom_default,&c_zoom_max, 
                                                             &i_parm_result,&ewkt_center);
      if (i_rc == RL2_OK)
      {
       zoom_min=(int)c_zoom_min;
       zoom_default=(int)c_zoom_default;
       zoom_max=(int)c_zoom_max;
       // printf ("-I-> fnct_GetRasterCoverageDefaults command_type[%d,%d] parm_type[%d] text[%s] rc=%d  \n",command_type,parm_command,parm_type,ewkt_center,i_rc); 
       if (parm_type == 0)
       {
        if (ewkt_center != NULL)
        {
         sql=sqlite3_mprintf("%2.7f;%d;%d,%d,%d;%s",parm_res_x,parm_srid,zoom_min,zoom_default,zoom_max,ewkt_center);
        }
        else
         sql=sqlite3_mprintf("%2.7f;%d;%d,%d,%d",parm_res_x,parm_srid,zoom_min,zoom_default,zoom_max);
        i_length = strlen (sql);
       }
       if (i_length > 0)
       {
        converage_defaults_center=sqlite3_malloc(i_length+1);		
        strcpy (converage_defaults_center, sql);
        sqlite3_free(sql);
        sql=NULL;
       }
     ...
     break;
     case 2:
     { // Tasks using parm_command as first parameter
     }
     break;
    }  
    if (i_rc < 0)
     goto error;    
   switch (parm_type)
   { // Setting result, based on parm type
    // a csv similar type result string
    case 0: 
     // Do not use sqlite3_free, causes crashes on android
     sqlite3_result_text (context, converage_defaults_center, strlen(converage_defaults_center), SQLITE_TRANSIENT);     
    break;
    case 1: 
    // return result of Geometry of Center Point only
     sqlite3_result_blob (context, blob_center, blob_center_sz, free);
    break;
    case 2: 
    // return result of 'ewkt' of Center Point only
     sqlite3_result_text (context, ewkt_center, strlen(ewkt_center), SQLITE_TRANSIENT);     
     if (converage_defaults_center != NULL)
     { // free only if not set to result
      sqlite3_free(converage_defaults_center);
      converage_defaults_center=NULL;
     }
    break;
    // return result of 'zoom_min' only
    case 3: 
     sqlite3_result_int (context, zoom_min);
    break;
    // return result of 'zoom_default' only
    case 4: 
     sqlite3_result_int (context, zoom_default);
    break;
    // return result of 'zoom_max' only
    case 5: 
     sqlite3_result_int (context, zoom_max);
    break;
   }
   if (cvg != NULL)
  ...
  error:    
    ...
    if (i_debug > 0)
    {
     printf ("-I-> fnct_GetRasterCoverageDefaults command_type[%d,%d] parm_type[%d] text[%s] rc=%d \n",command_type,parm_command,parm_type,coverage_name,i_rc); 
     // if ((coverage_name != NULL) && (i_rc > -3) && (i_rc < 0))
    ...
    switch (i_rc)
    {
     ...
     case RL2_OK: // 0
     break;
     // (-1 to -9 parameter errors, -10 to -99) from fnct_GetRasterCoverageDefaults
     // 1st parameter not TEXT,FLOAT or INTEGER
     case -1:
     // 2nd (optional Parameter) not an integer
     case -2:
     // 1st Parameter, coverage_name, is NULL
     case -3:
     // 1st Parameter, parm_res_x, is 0.0
     case -4:
     // 1st Parameter, parm_command, not between 0-5
     case -5:
     // 2nd Parameter,parm_type of command_type=0,1, not between 0-5
     case -6:
     // 2nd Parameter,parm_type of command_type=2, not between 0-?
     case -7:
     // invalid coverage name [rl2_create_coverage_from_dbms]
     case -10:
     // could not retrieve RasterDefaults pointer from coverage
     case -11:
     // sql prepared for SELECT MakePoint and  AsEWKT values has failed
     case -12:
     // retriving the values from SELECT MakePoint and  AsEWKT values has failed
     case -13:
     // At least one of the retrived  values from SELECT MakePoint and  AsEWKT values are NULL
     case -14:
     // (-101 - -199) from rl2_get_raster_coverage_stats
    ...
    sqlite3_result_int (context,i_rc);
    return;
}
...
/**
 ...
 \note Both the 'Center Point' and 'Zoom Levels' 
  - are convenience values only
  -- they are otherwise NOT used inside the Library
  - The intention is to offer Zoom-Level driven applications the capability
  -- to orientate themselves with information 
  --- about a starting point and renderable Zoom-Levels, nothing more 
  - The Database-Designer
  -- should use this function to set
  --- a specific starting point
  --- a 'best' viewing resolution
  --- a 'optimized' minimal/maximum viewing resolution
 \note Center Point [default: Center of the Raster Coverage]
   - will be stored using the srid of the raster_coverage
   - must be inside the extent of the raster_coverage
  \note Center Point : Geometry Type
   - if a non-POINT is used
   -- the center of that Geometry will be used
   - if the center is outside the raster_coverage
   -- the function will FAIL (even if the  'Zoom Levels' are correct)
   \note Center Point : srid
   - if no srid is used
   -- the srid of the raster_coverage is assumed
   --- the result MUST be inside the extent of the raster_coverage
   - if -1 as srid is used
   -- a pixel value is assumed, being the srid of the raster_coverage
   - if another srid is used
   -- it will be transormed using the srid of the raster_coverage
   --- the result MUST be inside the extent of the raster_coverage
 \note Restore Default Values [0,1,2]
     - 0: Restore 'Center Point' and 'Zoom Levels'
     - 1: Restore 'Center Point' only
     - 2: Restore 'Zoom Levels' only
  \note Zoom Levels Values [0-30]
     - zoom_min: MUST be >= 0 and smaller than 'zoom_default'
     - zoom_default: MUST be smaller than 'zoom_max'
     - zoom_max: MUST be <= 30 and bigger than 'zoom_default'
     - for all 3 values, when < 0 OR > 30:
     -- the value will be IGNORED (i.e the present value will be retained)
     - for all 3 values, when they share the same valie (i.e. zoom_min == zoom_default)
     -- the function will FAIL (even if the 'Center Point' is correct)
 \note Order and Rules of Zoom Levels Parameters [min/default/max : 0-30] 
    - smallest/bigger/biggest: none the same
 \note Zoom Levels [default: Zoom Level corresponding to the 'horz_resolution' value of the raster_coverage]
    - 0-30 are supported, 31 not being 'solvable' when using gdal
 \note Default setting after a Raster has been imported (RL2_LoadRaster* functions) 
    - zoom_min = 'horz_resolution' / 8
    - zoom_default = 'horz_resolution'
    - zoom_max = 'horz_resolution' * 8
 \note Default setting after a Raster has been imported (RL2_Pyramidize* functions) 
    - zoom_min = 'horz_resolution' / 8
    - zoom_default = 'horz_resolution'=x_resolution_1_1 of pyramind_level 0
    - zoom_max = x_resolution_1_8 of the higest pyramind_level
    
    JavaSqliteDescription[sqlite[3.8.10.2],spatialite[4.4.0-topoalpha],proj4[Rel. 4.9.1, 04 March 2015],geos[3.5.0-CAPI-1.9.0 r4084],lwgeom[2.2.0dev],spatialite_properties[HasIconv[1],HasMathSql[1],HasGeoCallbacks[0],HasProj[1],HasGeos[1],HasGeosAdvanced[1],HasGeosTrunk[0],HasLwGeom[1],HasLibXML2[1],HasEpsg[1],HasFreeXL[0],HasGeoPackage[1],target_cpu[armeabi-v7a]],rasterlite2_properties[1.6.10]]] recovery_mode_[STRICT]
 
 */
static void
fnct_SetRasterCoverageDefaults (sqlite3_context * context, int argc,
			     sqlite3_value ** argv)
{
/* SQL function:
/ SetRasterCoverageDefaults(String coverage_name, int reset parm)
/ SetRasterCoverageDefaults(String coverage_name, BLOB center_point)
/ SetRasterCoverageDefaults(String coverage_name, int zoom_min, int zoom_default,int zoom_max)
/ SetRasterCoverageDefaults(String coverage_name, BLOB center_point int zoom_min, int zoom_default,int zoom_max)
/
/ parm_type 
// 0 when a single BLOB is being used to set the center point
// 1 when a 3 integers are being used to set the Zoom-Levels
/
...
*/
    ...
    rl2PrivRasterDefaultsPtr coverage_raster_defaults=NULL;
    int coverage_srid=0;
    unsigned char zoom_min=255;
    unsigned char zoom_default=255;
    unsigned char zoom_max=255;
    double center_x=0.0;
    double center_y=0.0;
    int center_srid=0;
    double min_x=0.0;
    ...
    int i_parm_reset=1;
    int i_parm_min=1;
    int i_parm_default=2;
    int i_parm_max=3;
    unsigned char *blob_center;
    int blob_center_sz;
    RL2_UNUSED ();		/* LCOV_EXCL_LINE */
    if (sqlite3_value_type (argv[0]) != SQLITE_TEXT)
    { // invalid first parameter [not TEXT]
	    i_rc=-1;  
	    goto error;
    }
    if ((sqlite3_value_type (argv[1]) != SQLITE_BLOB) && (sqlite3_value_type (argv[1]) != SQLITE_INTEGER))
    { // invalid second parameter [not BLOB or INTEGER]
	    i_rc=-2;  
	    goto error;
    }
    else
    {
     ...
     if (argc > 2)
     {
      ...
      { // unsupported parm type 3 [not BLOB, INT,INT,INT]
       i_rc=-3;
      }
     }  
    }
    coverage_name = (const char *) sqlite3_value_text (argv[0]);
    if ((parm_type < 0) || (parm_type > 3))
    { // unsupported parm type [not 0-3]
     i_rc=-4;
     goto error;
    }
    if (parm_type == 0)
    {
     i_parm_reset = sqlite3_value_int (argv[1]);
     if ((i_parm_reset < 0) || (i_parm_reset > 2))
     { // unsupported reset parm [not 0-2]
      i_rc=-5;
      goto error;
     }     
    }   
   ...
   if ((parm_type == 1) || (parm_type == 3))
   { // SELECT RL2_GetRasterCoverageDefaults('1926.berlin_eiskeller.cutline') 
    // SELECT RL2_SetRasterCoverageDefaults('1926.berlin_eiskeller.cutline',MakePoint(6820.87,28707.18,3068)) 
    // SRID=3068;POLYGON((6000 27100,8800 27100,8800 30999.99999999999,6000 30999.99999999999,6000 27100))
    // SELECT RL2_SetRasterCoverageDefaults('1926.berlin_eiskeller.cutline',-1,21,-1) 
    // SELECT RL2_SetRasterCoverageDefaults('1926.berlin_eiskeller.cutline',MakePoint(6820.87,28707.18,3068),-1,18,-1) 
    // SELECT ST_SRID(MakePoint(6820.87,28707.18))
    // SELECT RL2_SetRasterCoverageDefaults('1926.berlin_eiskeller.cutline',0) 
    blob_center = (unsigned char*)sqlite3_value_blob (argv[1]);
	   blob_center_sz = sqlite3_value_bytes (argv[1]);
    if (blob_center_sz > 0)
    { // If the center-geometry was created without srid, the coverage srid will be used
     sql=sqlite3_mprintf("SELECT ST_MinX(ST_Envelope(?)), ST_MinY(ST_Envelope(?)),ST_MaxX(ST_Envelope(?)), ST_MaxY(ST_Envelope(?)),ST_SRID(?)");
     i_sql_rc = sqlite3_prepare_v2 (sqlite, sql, strlen (sql), &stmt_ewkt, NULL);      
     if (i_sql_rc == SQLITE_OK)
     {
      i_rc=-12;
      sqlite3_reset (stmt_ewkt);
      sqlite3_clear_bindings (stmt_ewkt);
      sqlite3_bind_blob (stmt_ewkt, 1,blob_center,blob_center_sz,free);
      sqlite3_bind_blob (stmt_ewkt, 2,blob_center,blob_center_sz,free);
      sqlite3_bind_blob (stmt_ewkt, 3,blob_center,blob_center_sz,free);
      sqlite3_bind_blob (stmt_ewkt, 4,blob_center,blob_center_sz,free);
      sqlite3_bind_blob (stmt_ewkt, 5,blob_center,blob_center_sz,free);
      while (1)
      {
       i_sql_rc = sqlite3_step (stmt_ewkt);
	      if (i_sql_rc == SQLITE_DONE)
       ...
       if (i_sql_rc == SQLITE_ROW)
	      {
        min_x = sqlite3_column_double (stmt_ewkt, 0);
        min_y = sqlite3_column_double (stmt_ewkt, 1);
        max_x = sqlite3_column_double (stmt_ewkt, 2);
        max_y = sqlite3_column_double (stmt_ewkt, 3);
        center_srid=sqlite3_column_int (stmt_ewkt, 4);        
        if ((min_x != max_x) && (min_y != max_y))
        { // LINESTRING,POLYGON and MULTI*,GEOMETRYCOLLECTION
     ...
     { // sql prepared for SELECT min/max values has failed
      i_rc=-11;
      printf ("-E-> fnct_SetRasterCoverageDefaults[%s] prepared failed for sql[%s] \n",coverage_name,sql); 
     }
     sqlite3_free(sql);
     sql=NULL;
    }
    else
    { // Retrieved Blob of center point is invalid
     i_rc=-10;
    }
   }
   if ((parm_type == 2) || (parm_type == 3))
   { // the unchanged zoom values (i.e. 255) will not be changed
    coverage_srid = sqlite3_value_int (argv[i_parm_min]);
    if ((coverage_srid >= 0) && (coverage_srid <= 30))
     zoom_min=(unsigned char)coverage_srid;
    coverage_srid = sqlite3_value_int (argv[i_parm_default]);
    if ((coverage_srid >= 0) && (coverage_srid <= 30))
     zoom_default=(unsigned char)coverage_srid;
    coverage_srid = sqlite3_value_int (argv[i_parm_max]);
    if ((coverage_srid >= 0) && (coverage_srid <= 30))
     zoom_max=(unsigned char)coverage_srid;
   }
   if ( i_rc < 0)
    goto error;
    //coverage_stats
   ...
   { // SELECT RL2_GetRasterCoverageDefaults('1926.berlin_eiskeller.cutline',0) 
    ...
    if (coverage_raster_defaults != NULL)
    {
     if (parm_type == 0)
     { 
      if (rl2_get_zoom_levels_for_resolutions(sqlite,coverage_name,coverage_srid,x_res,y_res,center_x,center_y,
                                                                          &zoom_min,&zoom_default,&zoom_max,NULL,NULL) == RL2_OK)
      {
       if ((i_parm_reset == 0) || (i_parm_reset == 1))
       { // will be retrieved from extent in rl2_set_raster_defaults_stats
        coverage_raster_defaults->center_x=0.0;
     ...
     if ((parm_type == 1) || (parm_type == 3))
     { // Possible ST_TRANSFORM will be done in 'rl2_set_raster_defaults_stats'
      coverage_raster_defaults->center_x=center_x;
      coverage_raster_defaults->center_y=center_y;
     }
     if (parm_type == 2)
     { // center-point being being retained     
     ...
     // coverage_stats will destroyed and set to NULL in 'rl2_set_raster_defaults_stats'     
     i_rc=rl2_set_raster_defaults_stats(sqlite,coverage_name,coverage_stats,center_srid);
     if (i_rc != RL2_OK)
     { // rl2_set_raster_defaults_stats has failed
      i_rc+=-200;
     }     
    }
    else
   ...
   return;
  error:
    printf ("-I-> fnct_SetRasterCoverageDefaults[%s,%d] error rc=%d \n",coverage_name,parm_type,i_rc); 
    if (cvg != NULL)
	    rl2_destroy_coverage (cvg);
    switch (i_rc)
    {
     //  (0) rl2_set_raster_defaults_stats
     case RL2_OK:
     i_rc=1; // 1 compleated correctly, with UPDATE
     break;
     //  (-201 - -299) rl2_set_raster_defaults_stats
     // nothing has changed, no UPDATE needed
     case -205:
      i_rc=RL2_OK; // 0 NOT considered as an error
     break;
     // (-1 to -9 parameter errors, -10 -99) from fnct_SetRasterCoverageDefaults
     // invalid first parameter [not TEXT]
     case -1:
     // invalid second parameter [not BLOB or INTEGER]
     case -2:
     ...
     // sql prepared for SELECT min/max values has failed
     case -11:
     // no result from SELECT ST_MinX(ST_Envelope(
     case -12:
     // could not retrieve RasterDefaults pointer from coverage
     case -13:
     // (-101 - -199) from rl2_get_raster_coverage_stats     
...
/		   String abstract)
/ SetRasterCoverageInfos(String coverage_name, String title, 
/		   String abstract, String copyright)
/
/ inserts or updates the descriptive infos supporting a Raster Coverage
/ returns 1 on success
/ 0 on failure, -1 on invalid arguments
*/
    ...
    if (argc > 2)
    {
     copyright = (const char *) sqlite3_value_text (argv[3]);
    }
    ret = set_coverage_infos (sqlite, coverage_name, title, abstract,copyright);
    sqlite3_result_int (context, ret);
}
...
/* preparing the WMS requests */
    ...
    if (rl2_get_defaults_stats(sqlite, section_stats,cvg_name,srid,(minx+((maxx-minx)/2)),(miny+((maxy-miny)/2))))
     goto error;
...
/* common implementation for Write GeoTIFF */
    ...
    *geom_cutline_blob_sz=0;
    ...
    if (by_section)
	  ...
	  if (argc > 10 && sqlite3_value_type (argv[10]) != SQLITE_INTEGER)
	      err = 1;
   // blob_cutline and crop_to_cutline
   if (argc > 11 && sqlite3_value_type (argv[11]) != SQLITE_BLOB)
	      err = 1;
   if (argc > 12 && sqlite3_value_type (argv[12]) != SQLITE_INTEGER)
	      err = 1;
	  ...
	  if (argc > 9 && sqlite3_value_type (argv[9]) != SQLITE_INTEGER)
	      err = 1;
   // blob_cutline and crop_to_cutline
   if (argc > 10 && sqlite3_value_type (argv[10]) != SQLITE_BLOB)
	      err = 1;
   if (argc > 11 && sqlite3_value_type (argv[11]) != SQLITE_INTEGER)
	      err = 1;
      }
...
/* retrieving all arguments */
    if (by_section)
	  ...
	  if (argc > 9)
       ...
       if (argc > 11)
	      {
        blob_cutline = sqlite3_value_blob (argv[11]);
	       blob_cutline_sz = sqlite3_value_bytes (argv[11]);       
       }
       if (argc > 12)
	       crop_to_cutline = sqlite3_value_int (argv[12]);
      }
    else
      {
	  /* whole Coverage */
	  ...
	  if (argc > 8)
       ...
       if (argc > 10)
	      {
        blob_cutline = sqlite3_value_blob (argv[10]);
	       blob_cutline_sz = sqlite3_value_bytes (argv[10]);
       }
       if (argc > 11)
	       crop_to_cutline = sqlite3_value_int (argv[11]);
      }
   sqlite = sqlite3_context_db_handle (context);
...
/* attempting to load the Coverage definitions from the DBMS */
    ...
    int i_srid=-1;
    rl2_get_coverage_srid (coverage, &i_srid);
     // Attempt to resolve missing parameters, were possible,  - the pointer (*geom_cutline_blob)
     //  stored in the indirect pointer (**geom_cutline_blob) will be != NULL, if a valid cutline geometry was supplied
    if (rl2_resolve_image_sanity(sqlite,cvg_name, blob, blob_sz, &minx, &miny, &maxx, &maxy,&horz_res,&vert_res,&width,&height,&pt_x,&pt_y,
         blob_cutline,blob_cutline_sz,crop_to_cutline,geom_cutline_blob,geom_cutline_blob_sz,i_srid) != RL2_OK)
    {
	  errcode = -1;
	  goto error;
      }
    if (by_section)
	  ...
	  ret =
	      rl2_export_section_geotiff_from_dbms (sqlite, max_threads, path,
						    ...
						    height, compression,
						    tile_sz, worldfile,*geom_cutline_blob,geom_cutline_blob_sz);
      }
    else
      {
	  /* whole Coverage */
	  ...
	  ret =
	      rl2_export_geotiff_from_dbms (sqlite, max_threads, path,
					    ...
					    worldfile,*geom_cutline_blob,geom_cutline_blob_sz);
      }
    if (ret != RL2_OK)
      {
	  errcode = 0;
...
/              double vert_res, int with_worldfile,
/              text compression, int tile_sz,
/              BLOB geom_cutline, int crop_to_cutline)
/
/ will return 1 (TRUE, success) or 0 (FALSE, failure)
/ or -1 (INVALID ARGS)
/
    ...
    *geom_cutline_blob_sz=0;
    ...
    if (by_section)
	  ...
	  if (argc > 13 && sqlite3_value_type (argv[13]) != SQLITE_INTEGER)
	      err = 1;
   // blob_cutline and crop_to_cutline
   if (argc > 14 && sqlite3_value_type (argv[14]) != SQLITE_BLOB)
	      err = 1;
   if (argc > 15 && sqlite3_value_type (argv[15]) != SQLITE_INTEGER)
	      err = 1;
      }
	  ...
	  if (argc > 12 && sqlite3_value_type (argv[12]) != SQLITE_INTEGER)
	      err = 1;
   // blob_cutline and crop_to_cutline
   if (argc > 13 && sqlite3_value_type (argv[13]) != SQLITE_BLOB)
	      err = 1;
   if (argc > 14 && sqlite3_value_type (argv[14]) != SQLITE_INTEGER)
	      err = 1;
      }
...
/* retrieving all arguments */
    if (by_section)
	  ...
	  if (argc > 12)
       ...
       if (argc > 14)
	      {
        blob_cutline = sqlite3_value_blob (argv[14]);
	       blob_cutline_sz = sqlite3_value_bytes (argv[14]);
       }
       if (argc > 15)
	       crop_to_cutline = sqlite3_value_int (argv[15]);
      }
    else
      {
	  /* whole Coverage */
	  ...
	  if (argc > 11)
       ...
       if (argc > 13)
	      {
        blob_cutline = sqlite3_value_blob (argv[13]);
	       blob_cutline_sz = sqlite3_value_bytes (argv[13]);
       }
       if (argc > 14)
	       crop_to_cutline = sqlite3_value_int (argv[14]);
      }
    ...
    *geom_cutline_blob_sz=0;
    ...
    if (by_section)
	  ...
	  if (argc > 11 && sqlite3_value_type (argv[11]) != SQLITE_INTEGER)
	      err = 1;
   // blob_cutline and crop_to_cutline
   if (argc > 12 && sqlite3_value_type (argv[12]) != SQLITE_BLOB)
	      err = 1;
   if (argc > 13 && sqlite3_value_type (argv[13]) != SQLITE_INTEGER)
	      err = 1;
      }
...
/* retrieving all arguments */
    if (by_section)
	  ...
	  if (argc > 10)
       ...
       if (argc > 12)
	      {
        blob_cutline = sqlite3_value_blob (argv[12]);
	       blob_cutline_sz = sqlite3_value_bytes (argv[12]);
       }
       if (argc > 13)
	       crop_to_cutline = sqlite3_value_int (argv[13]);
      }
    else
      {
	  /* whole Coverage */
...
/* common implementation Write TIFF */
   ...
   if (argc > 11 && sqlite3_value_type (argv[11]) != SQLITE_INTEGER)
	  ...
	  if (argc > 8 && sqlite3_value_type (argv[8]) != SQLITE_INTEGER)
	      err = 1;
       // blob_cutline and crop_to_cutline
   if (argc > 9 && sqlite3_value_type (argv[9]) != SQLITE_BLOB)
	      err = 1;
   if (argc > 10 && sqlite3_value_type (argv[10]) != SQLITE_INTEGER)
	      err = 1;
      }
...
/* retrieving all arguments */
    if (by_section)
	  ...
	  if (argc > 7)
       ...
       if (argc > 9)
	      {
        blob_cutline = sqlite3_value_blob (argv[9]);
	       blob_cutline_sz = sqlite3_value_bytes (argv[9]);
       }
       if (argc > 10)
	       crop_to_cutline = sqlite3_value_int (argv[10]);
      }
...
/* common implementation for Write JPEG */
   ...
   if (argc > 10 && sqlite3_value_type (argv[10]) != SQLITE_INTEGER)
	  ...
	  if (argc > 7 && sqlite3_value_type (argv[7]) != SQLITE_INTEGER)
	      err = 1;
   // blob_cutline and crop_to_cutline
   if (argc > 8 && sqlite3_value_type (argv[8]) != SQLITE_BLOB)
	      err = 1;
   if (argc > 9 && sqlite3_value_type (argv[9]) != SQLITE_INTEGER)
	      err = 1;
      }
...
/* retrieving all arguments */
    if (by_section)
	  ...
	  if (argc > 6)
       ...
       if (argc > 8)
	      {
        blob_cutline = sqlite3_value_blob (argv[8]);
	       blob_cutline_sz = sqlite3_value_bytes (argv[8]);
       }
       if (argc > 9)
	       crop_to_cutline = sqlite3_value_int (argv[9]);
      }
	  ...
	  if (with_worldfile)
		...
		ret =
		    ...
		    (sqlite, path, coverage, section_id, horz_res, vert_res,
		     ...
		     green_band, blue_band, compression, tile_sz,*geom_cutline_blob,geom_cutline_blob_sz);
	    }
	  else
	    {
		/* plain TIFF, no Worldfile */
...
/* attempting to load the Coverage definitions from the DBMS */
    ...
    // Attempt to resolve missing parameters, were possible,  - the pointer (*geom_cutline_blob)
    //  stored in the indirect pointer (**geom_cutline_blob) will be != NULL, if a valid cutline geometry was supplied
    if (rl2_resolve_image_sanity(sqlite,cvg_name, blob, blob_sz, &minx, &miny, &maxx, &maxy,&resolution,&resolution,&width,&height,&pt_x,&pt_y,
         blob_cutline,blob_cutline_sz,crop_to_cutline,geom_cutline_blob,geom_cutline_blob_sz,i_srid) != RL2_OK)
    {
	  errcode = -1;
	  goto error;
...
/
/ Note: no geom_cutline and crop_to_cutline planned for this, but possible
// - would probably be considered to slow
/
...
/* attempting to load the Coverage definitions from the DBMS */
    ...
    // Attempt to resolve missing parameters, were possible,  - the pointer (*geom_cutline_blob)
    //  stored in the indirect pointer (**geom_cutline_blob) will be != NULL, if a valid cutline geometry was supplied
    if (rl2_resolve_image_sanity(sqlite,cvg_name, blob, blob_sz, &minx, &miny, &maxx, &maxy,&x_res,&y_res,&width,&height,NULL,NULL,
         blob_cutline,blob_cutline_sz,crop_to_cutline,geom_cutline_blob,geom_cutline_blob_sz,srid) != RL2_OK)
    {
	    goto error;
    }
    ...
    if (by_section)
	  ...
	  if (rl2_get_raw_raster_data_mixed_resolutions
	      (sqlite, max_threads, coverage, base_width, base_height,
	       ...
	       bg_blue, symbolizer, stats,*geom_cutline_blob,geom_cutline_blob_sz) != RL2_OK)
	      goto error;
...
/* common implementation Export RAW Pixels */
   ...
   if (argc > 9 && sqlite3_value_type (argv[9]) != SQLITE_INTEGER)
	  ...
	  if (argc > 6 && sqlite3_value_type (argv[6]) != SQLITE_INTEGER)
	      err = 1;
   // blob_cutline and crop_to_cutline
   if (argc > 7 && sqlite3_value_type (argv[7]) != SQLITE_BLOB)
	      err = 1;
   if (argc > 8 && sqlite3_value_type (argv[8]) != SQLITE_INTEGER)
	      err = 1;
      }
...
/* retrieving all arguments */
    if (by_section)
	  ...
	  if (argc > 5)
       ...
       if (argc > 7)
	      {
        blob_cutline = sqlite3_value_blob (argv[7]);
	       blob_cutline_sz = sqlite3_value_bytes (argv[7]);
       }
       if (argc > 8)
	       crop_to_cutline = sqlite3_value_int (argv[8]);
      }
    ...
    if (by_section)
	  ...
	  ret =
	      rl2_export_section_raw_pixels_from_dbms (sqlite, max_threads,
						       ...
						       width, height,
						       big_endian, &xblob,
						       &xblob_sz,*geom_cutline_blob,geom_cutline_blob_sz);
      }
    else
      {
	  /* whole Coverage */
	  ret =
	      rl2_export_raw_pixels_from_dbms (sqlite, max_threads, coverage,
					       ...
					       big_endian, &xblob, &xblob_sz,*geom_cutline_blob,geom_cutline_blob_sz);
      }
    if (ret != RL2_OK)
	goto error;
    rl2_destroy_coverage (coverage);
...
register_rl2_sql_functions (void *p_db, const void *p_data)
{
    ...
    sqlite3_create_function (db, "ExportRawPixels", 9,
			     SQLITE_UTF8 | SQLITE_DETERMINISTIC, priv_data,
			     fnct_ExportRawPixels, 0, 0);
    sqlite3_create_function (db, "RL2_ExportRawPixels", 9,
			     SQLITE_UTF8 | SQLITE_DETERMINISTIC, priv_data,
			     fnct_ExportRawPixels, 0, 0);
    ...
    sqlite3_create_function (db, "RL2_ExportSectionRawPixels", 7,
			     SQLITE_UTF8 | SQLITE_DETERMINISTIC, priv_data,
			     fnct_ExportSectionRawPixels, 0, 0);
    // mj10777: this was ExportRawPixels. changed to ExportSectionRawPixels
    ...
    sqlite3_create_function (db, "RL2_ExportSectionRawPixels", 8,
			     SQLITE_UTF8 | SQLITE_DETERMINISTIC, priv_data,
			     fnct_ExportSectionRawPixels, 0, 0);
    sqlite3_create_function (db, "ExportSectionRawPixels", 9,
			     SQLITE_UTF8 | SQLITE_DETERMINISTIC, priv_data,
			     fnct_ExportSectionRawPixels, 0, 0);
    sqlite3_create_function (db, "RL2_ExportSectionRawPixels", 9,
			     SQLITE_UTF8 | SQLITE_DETERMINISTIC, priv_data,
			     fnct_ExportSectionRawPixels, 0, 0);
    sqlite3_create_function (db, "ExportSectionRawPixels", 10,
			     SQLITE_UTF8 | SQLITE_DETERMINISTIC, priv_data,
			     fnct_ExportSectionRawPixels, 0, 0);
    sqlite3_create_function (db, "RL2_ExportSectionRawPixels", 10,
			     SQLITE_UTF8 | SQLITE_DETERMINISTIC, priv_data,
			     fnct_ExportSectionRawPixels, 0, 0);
    ...
    sqlite3_create_function (db, "RL2_ImportSectionRawPixels", 9,
			     SQLITE_UTF8 | SQLITE_DETERMINISTIC, priv_data,
			     fnct_ImportSectionRawPixels, 0, 0);
    sqlite3_create_function (db, "GetRasterCoverageDefaults", 1,
				   SQLITE_UTF8,0, fnct_GetRasterCoverageDefaults, 0, 0);
	   sqlite3_create_function (db, "RL2_GetRasterCoverageDefaults", 1,
				   SQLITE_UTF8,0, fnct_GetRasterCoverageDefaults, 0, 0); 
    sqlite3_create_function (db, "GetRasterCoverageDefaults", 2,
				   SQLITE_UTF8,0, fnct_GetRasterCoverageDefaults, 0, 0);
	   sqlite3_create_function (db, "RL2_GetRasterCoverageDefaults", 2,    
				   SQLITE_UTF8,0, fnct_GetRasterCoverageDefaults, 0, 0);  
    sqlite3_create_function (db, "GetRasterCoverageDefaults", 3,
				   SQLITE_UTF8,0, fnct_GetRasterCoverageDefaults, 0, 0);
    sqlite3_create_function (db, "RL2_GetRasterCoverageDefaults", 3,    
				   SQLITE_UTF8,0, fnct_GetRasterCoverageDefaults, 0, 0); 
    sqlite3_create_function (db, "SetRasterCoverageDefaults", 1,
				   SQLITE_UTF8,0, fnct_SetRasterCoverageDefaults, 0, 0);
    sqlite3_create_function (db, "RL2_SetRasterCoverageDefaults", 1,
				   SQLITE_UTF8,0, fnct_SetRasterCoverageDefaults, 0, 0); 
   sqlite3_create_function (db, "SetRasterCoverageDefaults", 2,
				   SQLITE_UTF8,0, fnct_SetRasterCoverageDefaults, 0, 0);
    sqlite3_create_function (db, "RL2_SetRasterCoverageDefaults", 2,    
				   SQLITE_UTF8,0, fnct_SetRasterCoverageDefaults, 0, 0);
    sqlite3_create_function (db, "SetRasterCoverageDefaults", 4,
				   SQLITE_UTF8,0, fnct_SetRasterCoverageDefaults, 0, 0);
    sqlite3_create_function (db, "RL2_SetRasterCoverageDefaults", 4,
				   SQLITE_UTF8,0, fnct_SetRasterCoverageDefaults, 0, 0);
    sqlite3_create_function (db, "SetRasterCoverageDefaults", 5,
				   SQLITE_UTF8,0, fnct_SetRasterCoverageDefaults, 0, 0);
    sqlite3_create_function (db, "RL2_SetRasterCoverageDefaults", 5,
				   SQLITE_UTF8,0, fnct_SetRasterCoverageDefaults, 0, 0);
...
*/
   ...
   sqlite3_create_function (db, "WriteGeoTiff", 11, SQLITE_UTF8,
	  ...
	  sqlite3_create_function (db, "RL2_WriteGeoTiff", 11, SQLITE_UTF8,
				   priv_data, fnct_WriteGeoTiff, 0, 0);
   sqlite3_create_function (db, "WriteGeoTiff", 12, SQLITE_UTF8,
				   priv_data, fnct_WriteGeoTiff, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteGeoTiff", 12, SQLITE_UTF8,   
				   priv_data, fnct_WriteGeoTiff, 0, 0);       
   ...
   sqlite3_create_function (db, "WriteTiffTfw", 10, SQLITE_UTF8,
	  ...
	  sqlite3_create_function (db, "RL2_WriteTiffTfw", 10, SQLITE_UTF8,
				   priv_data, fnct_WriteTiffTfw, 0, 0);
   sqlite3_create_function (db, "WriteTiffTfw", 11, SQLITE_UTF8,
				   priv_data, fnct_WriteTiffTfw, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteTiffTfw", 11, SQLITE_UTF8,
				   priv_data, fnct_WriteTiffTfw, 0, 0);
   ...
   sqlite3_create_function (db, "WriteTiff", 10, SQLITE_UTF8, priv_data,
	  ...
	  sqlite3_create_function (db, "RL2_WriteTiff", 10, SQLITE_UTF8,
				   priv_data, fnct_WriteTiff, 0, 0);
   sqlite3_create_function (db, "WriteTiff", 11, SQLITE_UTF8, priv_data,
				   fnct_WriteTiff, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteTiff", 11, SQLITE_UTF8,
				   priv_data, fnct_WriteTiff, 0, 0);
   ...
   sqlite3_create_function (db, "WriteSectionGeoTiff", 12, SQLITE_UTF8,
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionGeoTiff", 12,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionGeoTiff, 0, 0);
   sqlite3_create_function (db, "WriteSectionGeoTiff", 13, SQLITE_UTF8,
				   0, fnct_WriteSectionGeoTiff, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionGeoTiff", 13,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionGeoTiff, 0, 0);
   ...
   sqlite3_create_function (db, "WriteSectionTiffTfw", 11, SQLITE_UTF8,
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionTiffTfw", 11,
				   SQLITE_UTF8, 0, fnct_WriteTiffTfw, 0, 0);
   sqlite3_create_function (db, "WriteSectionTiffTfw", 12, SQLITE_UTF8,
				   priv_data, fnct_WriteTiffTfw, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionTiffTfw", 12,
				   SQLITE_UTF8, 0, fnct_WriteTiffTfw, 0, 0);
   ...
   sqlite3_create_function (db, "WriteSectionTiff", 11, SQLITE_UTF8,
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionTiff", 11,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionTiff, 0, 0);
   sqlite3_create_function (db, "WriteSectionTiff", 12, SQLITE_UTF8,
				   priv_data, fnct_WriteSectionTiff, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionTiff", 12,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionTiff, 0, 0);
   ...
   sqlite3_create_function (db, "WriteJpegJgw", 9, SQLITE_UTF8,
	  ...
	  sqlite3_create_function (db, "RL2_WriteJpegJgw", 9, SQLITE_UTF8,
				   priv_data, fnct_WriteJpegJgw, 0, 0);
   sqlite3_create_function (db, "WriteJpegJgw", 10, SQLITE_UTF8,
				   priv_data, fnct_WriteJpegJgw, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteJpegJgw", 10, SQLITE_UTF8,
				   priv_data, fnct_WriteJpegJgw, 0, 0);
   ...
   sqlite3_create_function (db, "WriteJpeg", 9, SQLITE_UTF8, priv_data,
	  ...
	  sqlite3_create_function (db, "RL2_WriteJpeg", 9, SQLITE_UTF8,
				   priv_data, fnct_WriteJpeg, 0, 0);
   sqlite3_create_function (db, "WriteJpeg", 10, SQLITE_UTF8, priv_data,
				   fnct_WriteJpeg, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteJpeg", 10, SQLITE_UTF8,
				   priv_data, fnct_WriteJpeg, 0, 0);
   ...
   sqlite3_create_function (db, "WriteSectionJpegJgw", 10, SQLITE_UTF8,
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionJpegJgw", 10,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionJpegJgw, 0, 0);
   sqlite3_create_function (db, "WriteSectionJpegJgw", 11, SQLITE_UTF8,
				   priv_data, fnct_WriteSectionJpegJgw, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionJpegJgw", 11,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionJpegJgw, 0, 0);
   ...
   sqlite3_create_function (db, "WriteSectionJpeg", 10, SQLITE_UTF8,
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionJpeg", 10, SQLITE_UTF8,
				   priv_data, fnct_WriteSectionJpeg, 0, 0);
   sqlite3_create_function (db, "WriteSectionJpeg", 11, SQLITE_UTF8,
				   priv_data, fnct_WriteSectionJpeg, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionJpeg", 11, SQLITE_UTF8,
				   priv_data, fnct_WriteSectionJpeg, 0, 0);
   ...
   sqlite3_create_function (db, "WriteSectionTripleBandGeoTiff", 15,
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionTripleBandGeoTiff",
				   ...
				   fnct_WriteSectionTripleBandGeoTiff, 0, 0);
   sqlite3_create_function (db, "WriteSectionTripleBandGeoTiff", 16,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionTripleBandGeoTiff, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionTripleBandGeoTiff",
				   16, SQLITE_UTF8, priv_data,
				   fnct_WriteSectionTripleBandGeoTiff, 0, 0);
   ...
   sqlite3_create_function (db, "WriteMonoBandGeoTiff", 12,
	  ...
	  sqlite3_create_function (db, "RL2_WriteMonoBandGeoTiff", 12,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteMonoBandGeoTiff, 0, 0);
   sqlite3_create_function (db, "WriteMonoBandGeoTiff", 13,
				   SQLITE_UTF8, 0, fnct_WriteMonoBandGeoTiff,
				   0, 0);
	  sqlite3_create_function (db, "RL2_WriteMonoBandGeoTiff", 13,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteMonoBandGeoTiff, 0, 0);
   ...
   sqlite3_create_function (db, "WriteSectionMonoBandGeoTiff", 13,
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionMonoBandGeoTiff", 13,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionMonoBandGeoTiff, 0, 0);
   sqlite3_create_function (db, "WriteSectionMonoBandGeoTiff", 14,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionMonoBandGeoTiff, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionMonoBandGeoTiff", 14,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionMonoBandGeoTiff, 0, 0);
   ...
   sqlite3_create_function (db, "WriteTripleBandTiffTfw", 13,
	  ...
	  sqlite3_create_function (db, "RL2_WriteTripleBandTiffTfw", 13,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteTripleBandTiffTfw, 0, 0);
   sqlite3_create_function (db, "WriteTripleBandTiffTfw", 14,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteTripleBandTiffTfw, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteTripleBandTiffTfw", 14,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteTripleBandTiffTfw, 0, 0);
   ...
   sqlite3_create_function (db, "WriteSectionTripleBandTiffTfw", 14,
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionTripleBandTiffTfw",
				   14, SQLITE_UTF8, priv_data,
				   fnct_WriteSectionTripleBandTiffTfw, 0, 0);
   sqlite3_create_function (db, "WriteSectionTripleBandTiffTfw", 15,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionTripleBandTiffTfw, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionTripleBandTiffTfw",
				   15, SQLITE_UTF8, priv_data,
   ...
   sqlite3_create_function (db, "WriteSectionMonoBandTiffTfw", 12,
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionMonoBandTiffTfw", 12,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionMonoBandTiffTfw, 0, 0);
   sqlite3_create_function (db, "WriteSectionMonoBandTiffTfw", 13,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionMonoBandTiffTfw, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionMonoBandTiffTfw", 13,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionMonoBandTiffTfw, 0, 0);
   ...
   sqlite3_create_function (db, "WriteMonoBandTiffTfw", 11,
	  ...
	  sqlite3_create_function (db, "RL2_WriteMonoBandTiffTfw", 11,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteMonoBandTiffTfw, 0, 0);
   sqlite3_create_function (db, "WriteMonoBandTiffTfw", 12,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteMonoBandTiffTfw, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteMonoBandTiffTfw", 12,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteMonoBandTiffTfw, 0, 0);
   ...
   sqlite3_create_function (db, "WriteTripleBandTiff", 13, SQLITE_UTF8,
	  ...
	  sqlite3_create_function (db, "RL2_WriteTripleBandTiff", 13,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteTripleBandTiff, 0, 0);
   sqlite3_create_function (db, "WriteTripleBandTiff", 14, SQLITE_UTF8,
				   0, fnct_WriteTripleBandTiff, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteTripleBandTiff", 14,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteTripleBandTiff, 0, 0);
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionTripleBandTiff", 14,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionTripleBandTiff, 0, 0);
   sqlite3_create_function (db, "WriteSectionTripleBandTiff", 15,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionTripleBandTiff, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionTripleBandTiff", 15,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionTripleBandTiff, 0, 0);
   ...
   sqlite3_create_function (db, "WriteMonoBandTiff", 11, SQLITE_UTF8,
	  ...
	  sqlite3_create_function (db, "RL2_WriteMonoBandTiff", 11,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteMonoBandTiff, 0, 0);
   sqlite3_create_function (db, "WriteMonoBandTiff", 12, SQLITE_UTF8,
				   priv_data, fnct_WriteMonoBandTiff, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteMonoBandTiff", 12,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteMonoBandTiff, 0, 0);
   ...
   sqlite3_create_function (db, "WriteSectionMonoBandTiff", 12,
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionMonoBandTiff", 12,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionMonoBandTiff, 0, 0);
   sqlite3_create_function (db, "WriteSectionMonoBandTiff", 13,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionMonoBandTiff, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionMonoBandTiff", 13,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionMonoBandTiff, 0, 0);
   ...
   sqlite3_create_function (db, "WriteAsciiGrid", 9, SQLITE_UTF8,
	  ...
	  sqlite3_create_function (db, "RL2_WriteAsciiGrid", 9, SQLITE_UTF8,
				   priv_data, fnct_WriteAsciiGrid, 0, 0);
   sqlite3_create_function (db, "WriteAsciiGrid", 10, SQLITE_UTF8,
				   priv_data, fnct_WriteAsciiGrid, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteAsciiGrid", 10, SQLITE_UTF8,
				   priv_data, fnct_WriteAsciiGrid, 0, 0);
   ...
   sqlite3_create_function (db, "WriteSectionAsciiGrid", 10,
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionAsciiGrid", 10,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionAsciiGrid, 0, 0);
   sqlite3_create_function (db, "WriteSectionAsciiGrid", 11,
				   SQLITE_UTF8, 0, fnct_WriteSectionAsciiGrid,
				   0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionAsciiGrid", 11,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionAsciiGrid, 0, 0);
   ...
   sqlite3_create_function (db, "WriteNdviAsciiGrid", 11, SQLITE_UTF8,
	  ...
	  sqlite3_create_function (db, "RL2_WriteNdviAsciiGrid", 11,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteNdviAsciiGrid, 0, 0);
   sqlite3_create_function (db, "WriteNdviAsciiGrid", 12, SQLITE_UTF8,
				   priv_data, fnct_WriteNdviAsciiGrid, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteNdviAsciiGrid", 12,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteNdviAsciiGrid, 0, 0);
   ...
   sqlite3_create_function (db, "WriteSectionNdviAsciiGrid", 12,
	  ...
	  sqlite3_create_function (db, "RL2_WriteSectionNdviAsciiGrid", 12,
				   SQLITE_UTF8, priv_data,
		fnct_WriteSectionNdviAsciiGrid, 0, 0);
       sqlite3_create_function (db, "WriteSectionNdviAsciiGrid", 13,
				   SQLITE_UTF8, 0,
		 fnct_WriteSectionNdviAsciiGrid, 0, 0);
	  sqlite3_create_function (db, "RL2_WriteSectionNdviAsciiGrid", 13,
				   SQLITE_UTF8, priv_data,
				   fnct_WriteSectionNdviAsciiGrid, 0, 0);  
      }
}
