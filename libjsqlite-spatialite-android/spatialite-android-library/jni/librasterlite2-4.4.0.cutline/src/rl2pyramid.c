...
/* preparing the "tiles" SQL query */
    ...
    { // add cutline logic to sql (ignore all tiles that are completely outside of 'geom_cutline_blob')
     sql = sqlite3_mprintf("SELECT tile_id, MbrMinX(geometry), MbrMaxY(geometry),MbrMaxX(geometry), MbrMinY(geometry),ST_Contains(?,geometry) "
        "FROM \"%s\" " "WHERE pyramid_level = 0 AND section_id = ? AND ROWID IN ( "    
	       "SELECT ROWID FROM SpatialIndex WHERE f_table_name = %Q "
	       "AND search_frame = BuildMBR(?, ?, ?, ?) AND ST_Intersects(geometry,?))", xxtiles,xtiles);
     // mj10777: not called with RL2_WriteSectionGeoTiff
      // printf ("-I-> get_section_raw_raster_data cutline[%p] sz[%d] sql[%s]\n",geom_cutline_blob,*geom_cutline_blob_sz,sql);
    }
    else
    {
     sql = sqlite3_mprintf("SELECT tile_id, MbrMinX(geometry), MbrMaxY(geometry) "
...
/* preparing a raw pixels buffer */
	...
	(handle, max_threads, sect_id, stmt_tiles, stmt_data, bufpix, width,
	 ...
	 RL2_SCALE_1, palette, no_data,geom_cutline_blob,geom_cutline_blob_sz))
	goto error;
...
/* attempting to (re)build a 1,2,4-bit section pyramid from scratch */
	...
	(handle, max_threads, coverage, section_id, sect_width, sect_height,
	 ...
	 base_res_y, &inbuf, &inbuf_size, palette, no_data,NULL,NULL))
	goto error;
...
/* attempting to (re)build the section pyramid */
    ...
    rl2_get_zoom_levels_for_resolutions(handle,coverage,srid,res_x,res_y,
                                              (minx+((maxx-minx)/2)),(miny+((maxy-miny)/2)),
                                              &zoom_min,&zoom_default,&zoom_max,NULL,NULL);
		      printf ("  ----------\n");
		      printf
			  ("    %s: Monolithic Pyramid Level %d - Row %d of %d  successfully built. zoom_levels[%d,%d,%d]\n",
			   coverage, id_level + 1, row, tot_rows,zoom_min,zoom_default,zoom_max);
		  }
	    }
	  if (stop)
	      break;
