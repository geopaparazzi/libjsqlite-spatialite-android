...
/* creating a Raster Statistics (DBMS serialized format) */
    ...
    int raster_defaults_sz=21; // (1+(8+8+1+1+1)+1)
    int sz = 26;
    uLong crc;
    int ib;
    int ih;
    ...
    *ptr++ = RL2_DEFAULTS_START;
    raster_defaults=st->raster_defaults;
    exportDouble (ptr, raster_defaults->center_x, 1, endian_arch);
	  	ptr += 8;
    exportDouble (ptr, raster_defaults->center_y, 1, endian_arch);
	  	ptr += 8;
    *ptr++ =raster_defaults->zoom_min;
    *ptr++ =raster_defaults->zoom_default;
    *ptr++ =raster_defaults->zoom_max;
    *ptr++ = RL2_DEFAULTS_END;
/* computing the CRC32 */
    crc = crc32 (0L, p, ptr - p);
...
/* checking a Raster Statistics serialized object from validity */
   ...
   if (*ptr == RL2_DEFAULTS_START)
   { // Insures that Older-Blobs are still considered valid
    // skip over the structure (minus the END marker)
    ptr += (raster_defaults_sz-1);
    if (*ptr++ != RL2_DEFAULTS_END)
	    return 0;
   }
/* computing the CRC32 */
    crc = crc32 (0L, blob, ptr - blob);
...
/* attempting to deserialize a Raster Statistics object from DBMS binary format */
    ...
    if (*ptr == RL2_DEFAULTS_START)
    { // Insures that Older-Blobs are properly read
     ptr++;		/* skipping DEFAULTS_START marker */
     raster_defaults=st->raster_defaults;
     raster_defaults->center_x = importDouble (ptr, endian, endian_arch);	
     ptr += 8;
     raster_defaults->center_y = importDouble (ptr, endian, endian_arch);	
     ptr += 8;
     raster_defaults->zoom_min =*ptr++;
     raster_defaults->zoom_default = *ptr++;
     raster_defaults->zoom_max = *ptr++;
     ptr += 1;		/* skipping DEFAULTS_END marker */
    }
    return stats;
