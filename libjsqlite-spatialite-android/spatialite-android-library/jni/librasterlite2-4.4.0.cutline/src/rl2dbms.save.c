...
/* inserting into "raster_coverages" */
    ...
    sqlite3_bind_int (stmt, 11, srid);
      printf ("-W-> insert_into_raster_coverages[%s] no_data[%p] sample[%s,%d],pixel[%s,%d]  num_bands[%d]\n",coverage,blob_no_data,xsample,sample,xpixel,pixel,num_bands);
    ...
    ret = sqlite3_step (stmt);
          printf ("-W-> insert_into_raster_coveragesret=%d : [%s] no_data[%p] sample[%s,%d],pixel[%s,%d]  num_bands[%d]\n",ret,coverage,blob_no_data,xsample,sample,xpixel,pixel,num_bands);
...
/**
 ...
 \param Blob containing the Geometry, with size
 \param Address where to store the cloned copy of the created XY Geometry
 ...
 \note Goals:
  ...
  - return POLYGON or MULTIPOLYGON only

 \note this function will return a dynamically allocated buffer created 
 by gaiaCloneGeomColl(). 
 You are required to explicitly gaiaFreeGeomColl() any string returned by this function.
 */
static int
 ...
 *geom_cutline_blob_sz=0;
 if ((handle != NULL) && (blob_input != NULL) && (blob_input_sz > 0))
 {
   ...
   while (1)
   {
    ...
    {       
     ...
     if (sqlite3_column_type (stmt_cutline, 1) == SQLITE_TEXT)
	  	 {
	     s_geometry_type=(char *)sqlite3_column_text(stmt_cutline,1);
     }
  ...
  sqlite3_free (sql);
  if ((blob_cutline == NULL) || (blob_cutline_sz < 1))
   return i_geom_valid;
...
/**
 Attempt to resolve missing parameters
...
{ // 
 ...
 else
 {
  ...
  { //- (POINT,MULTIPOINT, non-closed LINESTRINGS, POLYGON that is only an Envelope are NOT considered valid)
   if (create_cutline_geometry(handle,blob_cutline, blob_cutline_sz, x_min, y_min, x_max, y_max,crop_to_cutline,geom_cutline_blob,geom_cutline_blob_sz,i_srid) == 1)
   { // rc=1 : Closed LINESTRÌNG / POLYGON (and MULTIs) or (not checked) GEOMETRYCOLLECTION)
 ...
 printf ("-I-> rl2_resolve_image_sanity[%d] [%d,%d,%d] image[%d,%d]  resolution[%2.4f,%2.4f] min_max[%2.4f,%2.4f,%2.4f,%2.4f] cutline[%p] sz[%d]\n",  i_rc, b_WidthHeight,b_Resolution,b_MinMax,*width,*height,*x_res,*y_res, *x_min, *y_min,*x_max, *y_max,*geom_cutline_blob,*geom_cutline_blob_sz);
 return i_rc;
}
...
/* servicing an AuxDecoder Tile request */
  ...
  decoder->geom_contains,decoder->geom_cutline_blob,decoder->geom_cutline_blob_sz))
      {
	  decoder->retcode = RL2_ERROR;
	  return;
      }
...
/* querying the tiles */
  ...
  //  geom_contains = 1: The tile is compleatly inside of the geometry
  int geom_contains = 1;
  ...
  // printf ("-I-> rl2_load_dbms_tiles_common geom_cutline_blob[%p] [%d] geom_contains[%d]\n",geom_cutline_blob,*geom_cutline_blob_sz,geom_contains);
  ...
  { // extract from the cutline a portion corresponding to the tile
   ...
   {  
    ...
    if (sqlite3_prepare_v2 (handle, sql, strlen(sql), &stmt_cutline, NULL) == SQLITE_OK)
    {
     ...
     i_rc = sqlite3_step (stmt_cutline);
     if ((i_rc == SQLITE_ROW) || (i_rc == SQLITE_DONE))
		   {
      if (sqlite3_column_type (stmt_cutline, 0) == SQLITE_BLOB)
	 	   {
       blob_cutline = sqlite3_column_blob (stmt_cutline, 0);
       blob_cutline_sz = sqlite3_column_bytes (stmt_data, 0);
       if (blob_cutline_sz > 0)
       {
        ...
        *decoder->geom_cutline_blob_sz=blob_cutline_sz;
        ...
        // printf ("-I-> rl2_load_dbms_tiles_common geom_cutline_blob[%p] [%d] sql[%s]\n",decoder->geom_cutline_blob,blob_cutline_sz,sql);
       }
      }
     }
    }  
...
/* preparing the "tiles" SQL query */
    ...
    sqlite3_free (xtiles);
    free (xxtiles);
    // printf ("-I-> rl2_get_raw_raster_data_common [stmt_tiles]: by_section[%d]  SELECT raw tiles SQL[%s]\n",by_section,  s_sql_statement);
...
/* attempting to return a buffer containing raw pixels from the DBMS Coverage + bgcolor */
	...
	rl2_get_raw_raster_data_common (handle, max_threads, cvg, 0, 0, width,
					...
					*out_pixel, no_data, xstyle, stats,NULL,NULL);
    if (no_data != NULL)
