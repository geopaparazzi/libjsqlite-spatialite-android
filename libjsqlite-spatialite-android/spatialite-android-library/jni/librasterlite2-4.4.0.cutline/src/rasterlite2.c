...
rl2_create_coverage (const char *name, unsigned char sample_type,
		     ...
		     rl2PixelPtr no_data,
       char* title,char* abstract,char* copyright)
{
...
/* applying the default policies */
    ...
    len = strlen (title);
    cvg->title = malloc (len + 1);
    strcpy (cvg->title, title);
    len = strlen (abstract);
    cvg->abstract = malloc (len + 1);
    strcpy (cvg->abstract, abstract);
    len = strlen (copyright);
    cvg->copyright = malloc (len + 1);
    strcpy (cvg->copyright, copyright);
...
/* memory cleanup - destroying a Coverage object */
   ...
   if (cvg->title != NULL)
	free (cvg->title);
   if (cvg->abstract != NULL)
	free (cvg->abstract);
  if (cvg->copyright != NULL)
	free (cvg->copyright);
    free (cvg);
}
...
RL2_DECLARE int
rl2_get_coverage_infos (rl2CoveragePtr ptr, char *title,
		       char *abstract, char*copyright)
{
/* return the Coverage title, abstract */
    rl2PrivCoveragePtr cvg = (rl2PrivCoveragePtr) ptr;
    if (cvg == NULL)
	return RL2_ERROR;
    title = cvg->title;
    abstract = cvg->abstract;
    copyright = cvg->copyright;
    return RL2_OK;
}
...
/* allocating and initializing a Raster Statistics object */
   ...
   stats->raster_defaults = malloc (sizeof (rl2PrivRasterDefaults));
   raster_defaults=stats->raster_defaults;
   raster_defaults->center_x = 0.0;
   raster_defaults->center_y = 0.0;
   raster_defaults->zoom_min = 0;
   raster_defaults->zoom_default = 18;
   raster_defaults->zoom_max = 30;
...
/* memory cleanup - destroying a Raster Statistics object */
	...
	free (st->band_stats);
    if (st->raster_defaults != NULL)
	    free (st->raster_defaults);
    free (st);
}
