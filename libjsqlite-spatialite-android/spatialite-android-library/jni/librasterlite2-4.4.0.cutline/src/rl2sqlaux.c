...
{
/* creating a default NO-DATA value [isTransparent = 1, so that we know that it a default-nodata]*/
    int nb;
    rl2PixelPtr pxl = rl2_create_pixel (sample, pixel, num_bands);
    if (pxl == NULL)
	return NULL;
    // do_import_file will only replace this with a gdal_nodata if (isTransparent==1)
    // - otherwise, for nodata,  isTransparent is not used
    rl2_set_pixel_transparent (pxl);
    switch (pixel)
      {
...
set_coverage_infos (sqlite3 * sqlite, const char *coverage_name,
		    const char *title, const char *abstract, const char *copyright)
{
...
/* updating the Coverage */
	...
	"WHERE Lower(coverage_name) = Lower(?)";
   // sql = "UPDATE raster_coverages SET title = ?, abstract = ?, copyright = ?"
	 //  "WHERE Lower(coverage_name) = Lower(?)";
    ret = sqlite3_prepare_v2 (sqlite, sql, strlen (sql), &stmt, NULL);
    if (ret != SQLITE_OK)
      {
    ...
    // sqlite3_bind_text (stmt, 3, copyright, strlen (copyright), SQLITE_STATIC);
...
RL2_PRIVATE int
set_coverage_nodata (sqlite3 * sqlite, const char *coverage_name,
		    rl2PixelPtr no_data)
{
/* auxiliary function: updates the Coverage descriptive infos */
    int ret;
...
/* checking if the Coverage already exists */
    ...
    if (ret != SQLITE_OK)
      {
	  fprintf (stderr, "SetCoverageNoData: \"%s\"\n",
		   sqlite3_errmsg (sqlite));
	  goto stop;
      }
    sqlite3_reset (stmt);
    ...
    if (!exists)
     goto stop;
     if (rl2_serialize_dbms_pixel(no_data, &blob, &blob_sz) != RL2_OK)
      goto stop;
/* updating the Coverage */
    sql = "UPDATE raster_coverages SET nodata_pixel = ? "
	"WHERE Lower(coverage_name) = Lower(?)";
    ret = sqlite3_prepare_v2 (sqlite, sql, strlen (sql), &stmt, NULL);
    if (ret != SQLITE_OK)
      {
	...
	fprintf (stderr, "SetCoverageNoData() error: \"%s\"\n",
		 sqlite3_errmsg (sqlite));
    sqlite3_finalize (stmt);
    return retval;
  stop:
