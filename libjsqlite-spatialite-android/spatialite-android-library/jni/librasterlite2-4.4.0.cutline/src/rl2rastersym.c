...
#endif
/**
 Return function to determine if POINT is within Cutline Geometry
 - used int functions 'copy_*_raw_pixels' WHEN
 -- a pixel is not already transparent and a cutline of the tile is supplied

 \param Database connection
 \param postion x of POINT
 \param postion y of POINT
 \param Blob containing the Geometry
 \param Size of Blob containing the Geometry
 
 \return 0=no, 1=yes (result of ST_Contains), -1= malformed parameter
 */
static int
cutline_contains_point (sqlite3 * handle,double position_x, double position_y, unsigned char* geom_cutline_blob,int *geom_cutline_blob_sz)
{
 int i_contains=0;
 if ((geom_cutline_blob != NULL) && (geom_cutline_blob_sz != NULL) && (*geom_cutline_blob_sz > 0))
 {
  if (handle != NULL) 
  {
   ...
   sql = "SELECT ST_Contains(?,MakePoint(?,?,ST_SRID(?)))";
   ...
   if ( i_rc == SQLITE_OK)
   {
    ...
    sqlite3_bind_double (stmt_cutline, 2, position_x);
    sqlite3_bind_double (stmt_cutline, 3, position_y);
    sqlite3_bind_blob (stmt_cutline, 4, geom_cutline_blob,*geom_cutline_blob_sz,SQLITE_STATIC);
    while (1)
    { // scrolling the result set rows 
     ...
     {	     
      if (sqlite3_column_type (stmt_cutline, 0) == SQLITE_INTEGER)
	     { 
       i_contains = sqlite3_column_int (stmt_cutline, 0);
      }        
     }
    }
...
/* copying INT8 raw pixels from the DBMS tile into the output image */
   ...
   if ((geom_cutline_blob != NULL) && (geom_contains == 0) && (transparent == 0))
   { // Only WHEN: not already transparent AND this tile is NOT fully contained inside the cutline              
    if (cutline_contains_point(handle,geo_x,geo_y,geom_cutline_blob,geom_cutline_blob_sz) == 0)
     transparent=1;
   }
      ...
      sqlite3 * handle)
{
    ...
    geom_contains,geom_cutline_blob,geom_cutline_blob_sz, handle);
	  return 1;
      case RL2_SAMPLE_INT16:
...
/* copying raw pixels into the output buffer */
	...
	return 0;
    tile_maxx=tile_minx+((double)tile_width*x_res);
    tile_miny=tile_maxy-((double)tile_height*y_res);
    if ((geom_cutline_blob != NULL) && (geom_contains == 0))
    { 
     // printf ("-I-> rl2_copy_raw_pixels[%d,%p] image[%d,%d] resolution[%2.4f,%2.4f] minx_maxx[%2.4f,%2.4f] tile_BuildMBR[%2.4f,%2.4f,%2.4f,%2.4f] tile_width_height[%d,%d] geom_cutline_blob[%p] size[%d]\n", geom_contains,geom_cutline_blob,width,height,x_res,y_res, minx, maxy,tile_minx, tile_maxy,tile_maxx,tile_miny,tile_width,tile_height,geom_cutline_blob,*geom_cutline_blob_sz);
    }
