...
/** add  TIFFTAG_GDAL_NODATA, plus any other that may be needed*/
static const TIFFFieldInfo rl2_tiffFieldInfo[] = {
        { TIFFTAG_GDAL_NODATA,	-1,-1, TIFF_ASCII,	FIELD_CUSTOM,
          TRUE,	FALSE,	(char*) "GDALNoDataValue" }
        };
static TIFFExtendProc rl2_parent_extender = NULL;  // In case we want a chain of extensions
#define N(a) (sizeof(a) / sizeof (a[0]))
static void rl2_registerCustomTIFFTags(TIFF *tif)
{ // Install the extended Tag field info
 int error = TIFFMergeFieldInfo(tif, rl2_tiffFieldInfo, N(rl2_tiffFieldInfo));
 if (rl2_parent_extender)
  (*rl2_parent_extender)(tif);
}
static void rl2_libtiff_with_custom_tags() 
{ // After calling this, 
  // TIFFSetField (destination->out, TIFFTAG_GDAL_NODATA,	destination->GDAL_NODATA); can be called
 if (!rl2_parent_extender) 
  rl2_parent_extender = TIFFSetTagExtender(rl2_registerCustomTIFFTags);
}
...
/* creating an uninitialized TIFF origin */
    ...
    if (origin == NULL)
	return NULL;
    // To support user defined TIFFTAG_* (such as 'TIFFTAG_GDAL_NODATA')
    // - can be called multiple times, but -before- a tiff is opened
    // -- afterwhich the user defined tags can be read or written
    rl2_libtiff_with_custom_tags() ;
    len = strlen (path);
    ...
    origin->GDAL_NODATA = NULL;
    origin->DOCUMENTNAME = NULL;
    origin->IMAGEDESCRIPTION = NULL;
    origin->COPYRIGHT= NULL;
    return origin;
}
...
/* memory cleanup - destroying a TIFF origin */
	...
	free (origin->proj4text);
     // These belong to the tiff-reader, do not free
     origin->GDAL_NODATA=NULL;
     origin->DOCUMENTNAME=NULL;
     origin->IMAGEDESCRIPTION=NULL;
     origin->COPYRIGHT=NULL;
...
/* tiled/striped layout */
    ...
    // gdal_nodata (private tag)
    TIFFGetField(origin->in, TIFFTAG_GDAL_NODATA, &origin->GDAL_NODATA);
    // DOCUMENTNAME can be used to set raster_coverages.title
    TIFFGetField(origin->in, TIFFTAG_DOCUMENTNAME, &origin->DOCUMENTNAME);
    // IMAGEDESCRIPTION can be used to set raster_coverages.abstract
    TIFFGetField(origin->in, TIFFTAG_IMAGEDESCRIPTION, &origin->IMAGEDESCRIPTION);
     // COPYRIGHT can be used to set raster_coverages.copyright (planned)
    TIFFGetField(origin->in, TIFFTAG_COPYRIGHT, &origin->COPYRIGHT);    
    /*
    printf ("-I-> init_tiff_origin: gdal_nodata[%s]\n", origin->GDAL_NODATA);   
    printf ("-I-> init_tiff_origin: title[%s]\n", origin->DOCUMENTNAME);   
    printf ("-I-> init_tiff_origin: abstract[%s]\n", origin->IMAGEDESCRIPTION);   
    printf ("-I-> init_tiff_origin: copyright[%s]\n", origin->COPYRIGHT); 
    */               
...
/* creating an uninitialized TIFF destination */
    ...
    destination->GDAL_NODATA = NULL;
    destination->DOCUMENTNAME = NULL;
    destination->IMAGEDESCRIPTION = NULL;
    destination->COPYRIGHT = NULL;    
    return destination;
}
...
/* memory cleanup - destroying a TIFF destination */
    ...
    if (destination->GDAL_NODATA != NULL)
    { // this belongs to us, this should be freed
     free (destination->GDAL_NODATA);
     destination->GDAL_NODATA=NULL;
    }
    if (destination->DOCUMENTNAME != NULL)
    { // this belongs to us, this should be freed
     free (destination->DOCUMENTNAME);
     destination->DOCUMENTNAME=NULL;
    }
    if (destination->IMAGEDESCRIPTION != NULL)
    { // this belongs to us, this should be freed
     free (destination->IMAGEDESCRIPTION);
     destination->IMAGEDESCRIPTION=NULL;
    } 
    if (destination->COPYRIGHT != NULL)
    { // this belongs to us, this should be freed
     free (destination->COPYRIGHT);
     destination->COPYRIGHT=NULL;
    }     
}
...
/* setting up the TIFF headers */
  ...
  header_done:
    ...
    if (destination->GDAL_NODATA)
    {
      TIFFSetField (destination->out, TIFFTAG_GDAL_NODATA,	destination->GDAL_NODATA);
    }   
    if (destination->DOCUMENTNAME)
      TIFFSetField (destination->out, TIFFTAG_DOCUMENTNAME,	destination->DOCUMENTNAME);
    if (destination->IMAGEDESCRIPTION)
      TIFFSetField (destination->out, TIFFTAG_IMAGEDESCRIPTION,	destination->IMAGEDESCRIPTION);
    if (destination->COPYRIGHT)
      TIFFSetField (destination->out, TIFFTAG_COPYRIGHT,	destination->COPYRIGHT);
...
/**
 Creates a string for the use with 'TIFFTAG_GDAL_NODATA' 

 \param no_data from the raster_coverage
 
 \return nothing

 \note  - GDAL_NODATA: only supports the same nodata for all bands
 \note  -- a result will only be returned when all bands of the raster_coverage use the same value
 \note  - a GDAL aware application (such as QGIS), based on this value, will show the area as transparent.
 */
static void
fetch_gdal_nodata (rl2PixelPtr no_data, char **gdal_nodata)
{ 
 if (no_data)
 {
  rl2PrivPixelPtr pixel;
  rl2PrivSamplePtr sample;
  pixel = (rl2PrivPixelPtr) no_data;
  char int8=SCHAR_MAX;
 	unsigned char uint8=UCHAR_MAX;
	 short int16=SHRT_MAX;
	 unsigned short uint16=USHRT_MAX;
	 int int32=INT_MAX;
	 unsigned int uint32=UINT_MAX;
	 float float32=FLT_MAX;
  int i_error=0;
  int band_id=0;
  char nodata_value[1024];
  for (band_id=0;band_id< pixel->nBands;band_id++)
  {
   ...
   switch (pixel->sampleType)
   {
    ...
    case RL2_SAMPLE_UINT8:
    {
     ...
     {
      uint8=sample->uint8;
      sprintf (nodata_value, "%d",uint8);
     }
     else
      if (uint8 != sample->uint8)
       i_error++;
    }
    break;
    case RL2_SAMPLE_INT16:
    {
     ...
     {
      int16=sample->int16;
      sprintf (nodata_value, "%d",int16);
     }
     else
      if (int16 != sample->int16)
       i_error++;
    }
    break;
    case RL2_SAMPLE_INT32:
     ...
     {
      int32=sample->int32;
      sprintf (nodata_value, "%d",int32);
     }
     else
      if (int32 != sample->int32)
       i_error++;
    }
	   break;
    case RL2_SAMPLE_UINT32:
    {
     if (uint32 == UINT_MAX)
     {
      ...
      sprintf (nodata_value, "%d",uint32);
     }
     else
      if (uint32 != sample->uint32)
       i_error++;
    }
	   break;
    case RL2_SAMPLE_FLOAT:
    ...
    {
     if (float32 == FLT_MAX)
     { // gdal: GTiffDataset::WriteNoDataValue
      float32=sample->float32;
      sprintf (nodata_value, "%.18g",float32); 
     } 
     else
      if (float32 != sample->float32)
       i_error++;
    }
    break;
   }
  ...
  if (i_error==0)
  {
   int32=strlen(nodata_value);
   *gdal_nodata = malloc (int32 + 1);
		 strcpy (*gdal_nodata,nodata_value);
  }
 }
}
/**
 Sets a raster_coverage no_data Pixel to the values from 'TIFFTAG_GDAL_NODATA' 

 \param no_data from the raster_coverage [MUST be the active coverage]
...
rl2_set_gdal_nodata (rl2PixelPtr no_data, char *gdal_nodata)
{ 
 rl2PixelPtr no_data_gdal=NULL;
 if ((no_data) && (gdal_nodata))
 {
  no_data_gdal=rl2_create_pixel (((rl2PrivPixelPtr)no_data)->sampleType, ((rl2PrivPixelPtr)no_data)->pixelType, ((rl2PrivPixelPtr)no_data)->nBands);
  rl2PrivPixelPtr pixel;
  rl2PrivSamplePtr sample;
  pixel = (rl2PrivPixelPtr) no_data_gdal;
  // whatever we recieve, this should work
  double float64= atof(gdal_nodata);
  unsigned int uint32=(unsigned int)float64;
  int int32=(int)float64;;
  float float32=0.0;
  char int8=0;
 	unsigned char uint8=0;
	 short int16=0;
	 unsigned short uint16=0;
  if ((float64 >= FLT_MIN) && ((float64 <= FLT_MAX))) 
   float32=(float)float64;
  if (uint32 <= UINT16_MAX)
   uint16=(unsigned short)uint32;
  if ((int32 >= INT16_MIN) && (int32 <= INT16_MAX))
   int16=(short)int32;
  if ((int16 >= INT8_MIN) && (int16 <= INT8_MAX))
   int8=(char)int16;
  if (uint16 <= UINT8_MAX)
   uint8=(char)uint16;
  int band_id=0;
  for (band_id=0;band_id< pixel->nBands;band_id++)
  {
   ...
   switch (pixel->sampleType)
   {
    ...
    {
     sample->uint8=uint8;
     // printf ("-I-> rl2_set_gdal_nodata[%d]: gdal_nodata[%d]\n", band_id,sample->uint8);   
    }
    break;
    case RL2_SAMPLE_INT16:
    {
    ...
    case RL2_SAMPLE_UINT32:
    {
     sample->uint32=uint32;
    }
	   break;
    case RL2_SAMPLE_FLOAT:
    {
     sample->float32=float32;
    }
    case RL2_SAMPLE_DOUBLE:
    {
...
/* attempting to create a file-based TIFF destination (no georeferencing) */
    ...
    if (destination == NULL)
	return NULL;
    fetch_gdal_nodata(no_data,&destination->GDAL_NODATA);  
    int len=0;   
    if (title != NULL)
    {
     len=strlen(title);
     destination->DOCUMENTNAME=malloc(len+1);
     strcpy(destination->DOCUMENTNAME,title);
    }
    if (abstract != NULL)
    {
     len=strlen(abstract);
     destination->IMAGEDESCRIPTION=malloc(len+1);
     strcpy(destination->IMAGEDESCRIPTION,abstract);
    }
    if (copyright != NULL)
    {
     len=strlen(copyright);
     destination->COPYRIGHT=malloc(len+1);
     strcpy(destination->COPYRIGHT,copyright);
    }
